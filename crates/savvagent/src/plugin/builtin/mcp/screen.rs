...
use savvagent_host::{ConnectState, ToolServerStatus};
...
impl McpManagerScreen {
    ...
    async fn submit_add(&mut self) -> Result<Vec<Effect>, PluginError> {
        ...
        let mut effects = vec![Self::push_note_effect(
            "Restart savvagent to apply changes.",
        )];
        ...
        self.skip_notes
            .insert(name, "restart savvagent to apply changes".to_string());
        self.mode = Mode::List;
        self.rebuild_rows();
        Ok(effects)
    }
    ...
    match live_status.map(|status| &status.state) {
        ...
        None => ServerRow {
            ...
            state_text: format!(
                ...
                skip_note
                    .cloned()
                    .unwrap_or_else(|| "restart savvagent to apply changes".to_string())
            ),
