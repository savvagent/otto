//! Grok SSE → SPP [`StreamEvent`] adapter.
//!
//! Grok (xAI)'s Chat Completions streaming endpoint emits `data:` SSE lines whose
...
use crate::api;
use crate::translate::{parse_tool_arguments, stop_reason_from_str, usage_from_grok};

/// Drive a Grok SSE streaming response to completion.
///
...
struct Accumulator {
    ...
    /// Per-Grok-tool-call-index → SPP block index mapping.
...
impl Accumulator {
    fn consume_chunk(&mut self, chunk: api::ChatCompletionChunk) -> Vec<StreamEvent> {
        ...
        if let Some(u) = chunk.usage {
            let spp_usage = usage_from_grok(u);
    ...
    fn flush(&mut self) -> Vec<StreamEvent> {
        ...
        if finish.unclosed_fence {
            tracing::warn!("grok stream ended with unclosed html-canvas fence");
        }
        ...
        // own Stop event mid-stream — Grok streams them as a series of
...
mod tests {
    ...
    async fn sse_decoder_errors_on_truncated_trailing_frame() {
        ...
        let bytes = bytes::Bytes::from_static(
            b"data: {\"id\":\"c1\",\"model\":\"grok-4-fast\",\"choices\":[{\"delta\":{\"content\":\"hi\"},\"finish_reason\":null}]}\n\ndata: {\"id\":\"c1\",\"model\":\"grok-4-fast\""
        );
