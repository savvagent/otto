//! Pure translation between SPP and Grok Chat Completions types.
//!
//! Grok differs from SPP in a few structural ways this module hides:
//!
//! 1. **System messages** are first-class role-bearing messages in Grok's
//!    schema, not a separate top-level field.
//! 2. **Tool-call identity.** SPP carries a `tool_use_id` linking each
//!    `ToolUse` to its matching `ToolResult`; Grok uses the same `id`
...
//!    Grok requires a dedicated `tool` role message with a `tool_call_id`.
...
/// Translate an SPP request into a Grok Chat Completions body.
pub fn request_to_grok(req: &spp::CompleteRequest, stream: bool) -> api::ChatCompletionRequest {
    ...
    let tools: Vec<api::Tool> = req.tools.iter().map(tool_to_grok).collect();
    // Intentionally leave `tool_choice` unset. Grok defaults to `"auto"`
...
/// Translate a non-streaming Grok response into an SPP [`spp::CompleteResponse`].
pub fn response_from_grok(r: api::ChatCompletionResponse) -> spp::CompleteResponse {
    let usage = r.usage.map(usage_from_grok).unwrap_or_default();
                    ...
                    spp::ContentBlock::ToolUse { id, name, input } => {
                        tool_calls.push(api::RequestToolCall {
                            ...
                            function: api::RequestFunction {
                                name: name.clone(),
                                // Grok expects arguments as a JSON string.
            ...
            for b in &m.content {
                match b {
                    ...
                    spp::ContentBlock::Image { source } => {
                        if let Some(part) = image_to_grok(source) {
...
fn image_to_grok(source: &spp::ImageSource) -> Option<api::ContentPart> {
...
fn tool_to_grok(t: &spp::ToolDef) -> api::Tool {
    api::Tool {
...
pub(crate) fn usage_from_grok(u: api::UsageStats) -> spp::Usage {
    spp::Usage {
...
mod tests {
    ...
    fn translates_simple_text_request() {
        let req = spp::CompleteRequest::text("grok-4-fast", "hello", 64);
        let body = request_to_grok(&req, false);
