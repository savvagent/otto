...
use otto_plugin_wasm::trust::{TrustCheck, TrustFile, tree_hash};
...
fn save_creates_intermediate_dirs() {
    // `home_dir` may not yet have a `.otto/` subdirectory; `save`
    // must create it on the way in.
    let tmp = tempfile::tempdir().unwrap();
    let mut tf = TrustFile::default();
    tf.trust("acme.demo", "abc".into(), None);
    tf.save(tmp.path()).unwrap();
    assert!(tmp.path().join(".otto/plugin-trust.toml").is_file());
}
