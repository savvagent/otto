...
use otto_plugin_wasm::discovery::{SourceScope, discover};
...
"#,
    ...
    assert_eq!(d.plugins[0].source_scope, SourceScope::UserOtto);
...
fn multiple_distinct_ids_all_returned() {
    ...
    for id in ["acme.one", "acme.two", "acme.three"] {
        let plugin_dir = home.join(".otto/plugins").join(id);
        std::fs::create_dir_all(&plugin_dir).unwrap();
        std::fs::write(
            plugin_dir.join("plugin.toml"),
            format!(
...
fn non_dir_entries_ignored() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let home = tmp.path();
    let plugins_root = home.join(".otto/plugins");
