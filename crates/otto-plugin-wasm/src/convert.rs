...
use crate::static_world::otto::plugin::types as wit;

use otto_plugin as sp;
use otto_plugin::manifest as spm;
use otto_plugin::styled as sps;
...
pub fn contributions_from_wit(c: wit::Contributions) -> spm::Contributions {
    // `Contributions` is `#[non_exhaustive]` in `otto-plugin` and that
