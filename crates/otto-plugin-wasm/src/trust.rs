...
impl TrustFile {
    /// Load the trust file from `<home_dir>/.otto/plugin-trust.toml`.
    ///
    ...
    /// Persist the trust file to `<home_dir>/.otto/plugin-trust.toml`,
...
mod tests {
    ...
    fn malformed_trust_file_errors_not_silently_empty() {
        let tmp = tempfile::tempdir().unwrap();
        let path = tmp.path().join(".otto/plugin-trust.toml");
