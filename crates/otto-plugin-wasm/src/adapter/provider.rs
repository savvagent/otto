...
//! [`otto_mcp::ProviderClient`] — the host-facing trait every provider
...
// `otto:plugin/types` is the shared (and now `with:`-aliased)
...
// `otto:plugin/spp` — bindgen-generated empty Host trait. The spp
...
impl http_wit::Host for ProviderHostState {
    ...
    async fn fetch_stream(
        ...
        Err(http_wit::HttpError::Transport(
            "fetch-stream is not supported by this host (otto v0.18.0)".to_string(),
        ))
    }
}

// `otto:plugin/http-capability/http-stream` — resource host impl.
...
// `otto:plugin/keyring-capability`
...
// `otto:plugin/progress-capability`
...
///
/// `count-tokens` has no [`otto_protocol`] counterpart, so we
...
pub struct CountTokensRequest {
    ...
    pub messages: Vec<otto_protocol::Message>,
}
