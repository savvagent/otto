//! Mechanical `From`/`Into` between `otto_protocol` SPP types and
//! the WIT bindings generated for the `plugin-provider` world.
//!
//! Round-trip discipline: every byte in a `otto_protocol` value must
...
//!   exports) and have no counterpart in `otto_protocol`. The provider
...
use crate::provider_world::otto::plugin::spp as wit;
    ...
    fn from(b: spp::ContentBlock) -> Self {
        match b {
            ...
            spp::ContentBlock::Html { source, state: _ } => {
                tracing::debug!(
                    target: "otto_plugin_wasm::spp_convert",
                    chars = source.len(),
...
// `otto_protocol::request::ThinkingConfig` path. We can't `use
...
mod roundtrip_tests {
    use super::*;
    use otto_protocol::request::ThinkingConfig;
    use otto_protocol::{
