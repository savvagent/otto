...
//!
//! Project paths beat user paths; within the same scope, `.otto/`
...
pub fn discover(project_root: Option<&Path>, home_dir: Option<&Path>) -> Discovery {
    ...
    let paths: Vec<(Option<PathBuf>, SourceScope)> = vec![
        (
            project_root.map(|p| p.join(".otto/plugins")),
            SourceScope::ProjectOtto,
        ),
        (
        ...
        (
            home_dir.map(|h| h.join(".otto/plugins")),
            SourceScope::UserOtto,
        ),
        (
