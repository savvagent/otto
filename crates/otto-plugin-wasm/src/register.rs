...
//!   otto crate owns that surface; this module just yields the data).
...
/// - `project_root`: where to look for `<project>/.otto/plugins/`
///   and `<project>/.claude/plugins/`. `None` skips the project tier
///   (useful for headless smoke tests).
/// - `home_dir`: where to look for `~/.otto/plugins/` and
...
mod tests {
    ...
    async fn register_external_untrusted_skipped_with_warning() {
        let tmp = tempfile::tempdir().expect("tempdir");
        let plugin_dir = tmp.path().join(".otto/plugins/acme.demo");
