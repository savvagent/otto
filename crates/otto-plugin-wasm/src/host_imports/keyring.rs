...
//! name is *not* a parameter — it's hardcoded to `"otto"`. That keeps
...
use crate::provider_world::otto::plugin::keyring_capability as wit;

/// Service identifier under which all otto-managed credentials live.
/// **Not** configurable by plugins; see the module docs for why.
pub const OTTO_KEYRING_SERVICE: &str = "otto";
...
impl KeyringState {
    ...
    /// Look up `account` under the otto service.
    ///
    /// Returns:
    ...
    pub fn get(&self, account: &str) -> Result<String, wit::KeyringError> {
        ...
        let entry = keyring::Entry::new(OTTO_KEYRING_SERVICE, account)
...
mod tests {
    ...
    #[test]
    fn otto_service_constant_is_stable() {
        // Locked at "otto" so the value matches what `/connect`
        ...
        assert_eq!(OTTO_KEYRING_SERVICE, "otto");
    }
}
