...
//!   output and `otto_plugin` types (Effect, Manifest, HookKind,
//!   ThemeColor, …; Task 4).
...
//!   as a `Box<dyn otto_plugin::Plugin>` (Task 4). Interactive and
...
///
/// The `with:` clause aliases the shared `otto:plugin/types`
...
pub mod interactive_world {
    wasmtime::component::bindgen!({
        ...
        async: true,
        with: {
            "otto:plugin/types@0.1.0": crate::static_world::otto::plugin::types,
        },
    });
}
...
/// `otto:plugin/types` interface to the *same* Rust types the static
