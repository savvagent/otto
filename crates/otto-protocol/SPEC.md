# Otto Provider Protocol (SPP) — v0.3.0

A small layering on top of the [Model Context Protocol] for exposing LLM
providers as MCP servers. Otto (the host) talks to provider servers over
...
1. Make adding a new model provider equivalent to writing a small standalone
   binary, with no changes to the Otto host.
...
The protocol version is exposed at `otto_protocol::SPP_VERSION`. Breaking
