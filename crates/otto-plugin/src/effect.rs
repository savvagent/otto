...
/// `otto_protocol::ToolDef` whose `input_schema: serde_json::Value`
...
pub enum Effect {
    ...
    SetActiveLocale {
        ...
        code: String,
        /// Whether to persist the selection to ~/.otto/config.toml.
        persist: bool,
    },
    ...
    /// use. The runtime fetches the client via a otto-internal seam (not
    ...
    /// to `~/.otto/plugins.toml`. Toggling a
