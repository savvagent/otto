...
//! (not a unit test) because `otto-host` declares
...
//! saves and restores the previous value of `OTTO_AGENT_MAX_DEPTH`
//! so it does not leak outside the locked section.

use std::sync::Mutex;

use otto_host::max_depth_from_env;
...
fn depth_limit_env_default_is_three() {
    ...
    // OTTO_AGENT_MAX_DEPTH, sibling tests serialize on ENV_LOCK,
    // and the previous value is restored before the guard drops.
    let prev = std::env::var("OTTO_AGENT_MAX_DEPTH").ok();
    unsafe {
    ...
    if let Some(v) = prev {
        unsafe {
            std::env::set_var("OTTO_AGENT_MAX_DEPTH", v);
        }
    }
}
...
fn depth_limit_env_override_parses() {
    ...
    unsafe {
        match prev {
            Some(v) => std::env::set_var("OTTO_AGENT_MAX_DEPTH", v),
            None => std::env::remove_var("OTTO_AGENT_MAX_DEPTH"),
        }
    }
}
