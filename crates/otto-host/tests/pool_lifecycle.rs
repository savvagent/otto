...
async fn set_active_provider_rejects_unknown() {
    ...
    assert!(matches!(err, otto_host::PoolError::NotRegistered(_)));
...
async fn force_disconnect_cooperative_cancel_short_circuits_grace() {
    ...
    let outcome = tokio::time::timeout(Duration::from_millis(200), turn_handle)
        ...
        .expect("JoinHandle should not panic");
    assert!(
        matches!(outcome, Err(otto_host::HostError::Cancelled(_))),
...
async fn force_disconnect_emits_cancelled_event_on_wire() {
    ...
    let (events_tx, mut events_rx) = mpsc::channel::<otto_host::TurnEvent>(32);
    ...
    while let Ok(event) = events_rx.try_recv() {
        if matches!(event, otto_host::TurnEvent::Cancelled { .. }) {
            saw_cancelled = true;
        }
    }
    assert!(
...
async fn force_disconnect_emits_aborted_after_grace_on_wire() {
    ...
    while let Ok(event) = events_rx.try_recv() {
        match event {
            otto_host::TurnEvent::AbortedAfterGrace { .. } => saw_aborted = true,
            otto_host::TurnEvent::Cancelled { .. } => saw_cancelled = true,
            _ => {}
        }
    }
...
async fn host_start_refuses_empty_post_filter_pool() {
    ...
    // `~/.otto/config.toml` carried `startup_providers=["anthropic"]`.
