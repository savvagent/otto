...
fn live_request_for(model: &str) -> otto_protocol::CompleteRequest {
...
/// `OTTO_ALLOW_MISSING_LIVE_KEYS=1` explicitly opts into the silent
...
fn require_live_key(primary: &str, fallback: Option<&str>) -> Option<String> {
    ...
    if std::env::var("OTTO_ALLOW_MISSING_LIVE_KEYS").is_ok() {
        eprintln!("{primary} not set; OTTO_ALLOW_MISSING_LIVE_KEYS active — skipping");
        return None;
    }
    let fallback_note = fallback.map(|f| format!(" (or {f})")).unwrap_or_default();
    panic!(
        "{primary}{fallback_note} not set, and OTTO_ALLOW_MISSING_LIVE_KEYS \
         is unset. Export the key, or set OTTO_ALLOW_MISSING_LIVE_KEYS=1 \
         to allow silent skip (local-dev convenience)."
    );
}

#[tokio::test]
#[ignore = "requires ANTHROPIC_API_KEY (or OTTO_ALLOW_MISSING_LIVE_KEYS=1 to skip silently); run with cargo test … -- --ignored"]
...
#[tokio::test]
#[ignore = "requires GEMINI_API_KEY (or GOOGLE_API_KEY, or OTTO_ALLOW_MISSING_LIVE_KEYS=1 to skip silently); run with cargo test … -- --ignored"]
...
#[tokio::test]
#[ignore = "requires OPENAI_API_KEY (or OTTO_ALLOW_MISSING_LIVE_KEYS=1 to skip silently); run with cargo test … -- --ignored"]
