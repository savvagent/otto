...
//!
//! - `OTTO_PROVIDER_URL` (default `http://127.0.0.1:8787/mcp`)
//! - `OTTO_MODEL`        (default `claude-haiku-4-5`)
//! - `OTTO_TOOL_FS_BIN`  (default: looked up on `$PATH` as `otto-tool-fs`)

use std::path::PathBuf;
use std::process::ExitCode;

use otto_host::{Host, HostConfig, ProviderEndpoint, ToolEndpoint};
...
async fn main() -> ExitCode {
    ...
    let url = std::env::var("OTTO_PROVIDER_URL")
    ...
    let tool_bin: PathBuf = std::env::var("OTTO_TOOL_FS_BIN")
        .map(PathBuf::from)
        .unwrap_or_else(|_| PathBuf::from("otto-tool-fs"));
