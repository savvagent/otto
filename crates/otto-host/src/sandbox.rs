...
//! [`apply_sandbox`]. User-pinned overrides in `~/.otto/sandbox.toml`
//! still win — set `[tool_overrides.tool-bash] allow_net = true` to opt
//! out of the prompt and grant net access unconditionally.
//!
//! # Config persistence
//!
//! `~/.otto/sandbox.toml` is loaded at startup (silent fallback on any
...
pub enum SandboxLoadStatus {
    /// `~/.otto/sandbox.toml` does not exist. Defaults are in use; the
...
/// Sandbox configuration. Persisted to `~/.otto/sandbox.toml`.
///
...
pub struct SandboxConfig {
    ...
    /// `~/.otto/sandbox.toml` — that override bypasses the prompt.
    pub allow_net: bool,
...
impl SandboxConfig {
    /// Load from `~/.otto/sandbox.toml`. Returns the default if the file
    ...
    /// Persist to `~/.otto/sandbox.toml`. Errors are propagated; the
                ...
                if cfg.version > SCHEMA_VERSION {
                    tracing::warn!(
                        "sandbox.toml at {} declares version {} but this build \
                         ...
                         Upgrade otto or remove the file to dismiss.",
                        path.display(),
                        cfg.version,
                        SCHEMA_VERSION,
                    );
...
/// `~/.otto/sandbox.toml`, or `None` if `$HOME` is unset.
fn sandbox_toml_path() -> Option<PathBuf> {
    let home = std::env::var_os("HOME")
        .or_else(|| std::env::var_os("USERPROFILE"))
        .map(PathBuf::from)?;
    Some(home.join(".otto").join("sandbox.toml"))
}
...
fn apply_linux(
    ...
    // sets `OTTO_TOOL_FS_ROOT`) keep their configuration.
...
mod tests {
    ...
    fn disabled_config_returns_none_wrapper() {
        let mut cmd = make_cmd("/usr/bin/otto-tool-fs");
        let wrapper = apply_sandbox(
            &mut cmd,
            Path::new("/usr/bin/otto-tool-fs"),
        ...
        assert_eq!(
            ...
            std::ffi::OsStr::new("/usr/bin/otto-tool-fs")
        );
    }
    ...
    fn default_config_denies_tool_bash_net_by_built_in_fallback() {
        ...
        assert!(
            !cfg.net_allowed_for(Path::new("/usr/bin/otto-tool-bash")),
            "v0.7 default-deny: bash gets no net unless host injects an override"
        );
        assert!(
            !cfg.net_allowed_for(Path::new("/usr/bin/otto-tool-fs")),
    ...
    fn non_bash_tool_inherits_global_net_setting() {
        ...
        let fs_bin = PathBuf::from("/usr/local/bin/otto-tool-fs");
    ...
    fn per_tool_override_can_deny_net_for_bash() {
        ...
        let bash_bin = PathBuf::from("/usr/local/bin/otto-tool-bash");
    ...
    mod linux {
        ...
        fn bwrap_argv_contains_required_flags() {
            ...
            let mut cmd = make_cmd("/usr/local/bin/otto-tool-fs");
            let cfg = SandboxConfig::default();
            let wrapper = apply_sandbox(
                &mut cmd,
                Path::new("/usr/local/bin/otto-tool-fs"),
        ...
        /// Regression: caller-supplied env vars (e.g. `OTTO_TOOL_FS_ROOT`)
        ...
        fn bwrap_preserves_caller_env_vars() {
            ...
            let mut cmd = make_cmd("/usr/local/bin/otto-tool-fs");
            cmd.env("OTTO_TOOL_FS_ROOT", "/foo");
            cmd.env("OTTO_TOOL_BASH_ROOT", "/bar");
            let cfg = SandboxConfig::default();
            let wrapper = apply_sandbox(
                &mut cmd,
                Path::new("/usr/local/bin/otto-tool-fs"),
            ...
            assert!(
                envs.iter()
                    .any(|(k, v)| k == std::ffi::OsStr::new("OTTO_TOOL_FS_ROOT")
        ...
        fn tool_bash_unshares_net_by_default() {
            ...
            let mut cmd = make_cmd("/usr/local/bin/otto-tool-bash");
            apply_sandbox(
                &mut cmd,
                Path::new("/usr/local/bin/otto-tool-bash"),
                Path::new("/tmp/project"),
                &cfg,
            );
            let args: Vec<String> = cmd
        ...
        async fn bwrap_sandboxed_cannot_read_shadow() {
            ...
            cmd.arg("-c")
                .arg("echo hello > /tmp/otto_sandbox_test_write_outside");
