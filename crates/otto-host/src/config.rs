...
pub enum ProviderEndpoint {
    ...
    /// `provider-anthropic`'s `otto-anthropic` binary listening on a
    /// loopback port).
    StreamableHttp {
...
pub struct HostConfig {
    ...
    pub max_tokens: u32,
    /// Project root, used to locate `OTTO.md` and as default cwd context.
    pub project_root: PathBuf,
    /// Optional embedder-supplied system-prompt layer. Composed (in
    /// order) after the built-in default prompt and before the
    /// `OTTO.md` body via [`crate::project::layered_prompt`].
    /// When `None`, only the default and `OTTO.md` layers
    ...
    ///
    /// When `None`, the host loads `~/.otto/sandbox.toml` via
...
impl HostConfig {
    ...
    /// `~/.otto/sandbox.toml` at startup. Sandboxing is enabled by
