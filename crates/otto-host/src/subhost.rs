...
/// Read `OTTO_AGENT_MAX_DEPTH` (default 3). Parse failures fall
...
/// inside `otto-host`; external constructors go through
/// [`SubHost::new`].
pub struct SubHost {
...
impl SubHost {
    ...
    pub async fn run_subagent(&self, prompt: String) -> Result<String, SubHostError> {
        use otto_protocol::{CompleteRequest, ContentBlock, Message, Role, StopReason};
    ...
    async fn dispatch_tool(&self, call: &PendingToolCall) -> otto_protocol::ContentBlock {
    ...
    async fn dispatch_inner(&self, call: &PendingToolCall) -> otto_protocol::ContentBlock {
        use otto_protocol::ContentBlock;
...
/// final answer.
fn finalize_text(content: &[otto_protocol::ContentBlock]) -> Result<String, SubHostError> {
    use otto_protocol::ContentBlock;
...
fn outcome_to_tool_result(id: &str, outcome: ToolCallOutcome) -> otto_protocol::ContentBlock {
    use otto_protocol::ContentBlock;
...
pub enum SubHostError {
    ...
    Cancelled,
    /// `OTTO_AGENT_MAX_DEPTH` would be exceeded by this dispatch.
