...
pub struct ToolRegistry {
    ...
    in_process: tokio::sync::RwLock<
        std::collections::HashMap<String, otto_plugin::InProcessToolHandlerArc>,
    >,
...
impl ToolRegistry {
    ...
    /// env vars — `OTTO_TOOL_FS_ROOT`, `OTTO_TOOL_BASH_ROOT`, and
    /// `OTTO_TOOL_GREP_ROOT` — so the bundled tool binaries confine
    ...
    pub async fn register_in_process_tool(
        &self,
        spec: ToolDef,
        handler: otto_plugin::InProcessToolHandlerArc,
    ) {
...
) -> Vec<otto_protocol::ContentBlock> {
...
pub(crate) struct ToolCallOutcome {
    ...
    pub blocks: Vec<otto_protocol::ContentBlock>,
}

impl ToolCallOutcome {
    pub(crate) fn success(payload: String) -> Self {
        let blocks = vec![otto_protocol::ContentBlock::Text {
            text: payload.clone(),
        }];
        Self {
            is_error: false,
...
mod tool_call_outcome_tests {
    ...
    fn non_html_resource_falls_back_to_text() {
        ...
        assert!(
            blocks
                .iter()
                .all(|b| !matches!(b, otto_protocol::ContentBlock::Html { .. }))
        );
    }
}
...
mod connect_tests {
    ...
    async fn connect_records_failed_endpoint_without_aborting_remaining_servers() {
        ...
        let endpoints = vec![
            ...
            ToolEndpoint::Stdio {
                name: "bogus".into(),
                command: PathBuf::from("definitely-not-a-real-otto-tool"),
                args: Vec::new(),
                env: HashMap::new(),
            },
        ];
