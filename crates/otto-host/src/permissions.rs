...
//! 2. **Front-matter rules** parsed from `OTTO.md` YAML front-matter
//!    (immutable for the session — project-pinned).
//! 3. **`~/.otto/permissions.toml`** rules (mutable — Always/Never
...
/// On-disk shape of `~/.otto/permissions.toml`.
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
...
/// Permissions section of `OTTO.md`'s YAML front-matter.
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
...
pub struct PermissionPolicy {
    ...
    /// Rules from `OTTO.md` front-matter — immutable for the session.
    front_matter_rules: Arc<Vec<Rule>>,
    /// Path to `~/.otto/permissions.toml`. `None` when `HOME` is unset.
...
impl PermissionPolicy {
    /// Build the default policy for `project_root`. Loads OTTO.md
    /// front-matter and `~/.otto/permissions.toml` if they exist;
    ...
    pub fn evaluate(&self, tool_name: &str, args: &Value) -> Verdict {
        ...
        if let Some(decision) = match_first(&self.front_matter_rules, tool_name, args) {
            return self.decision_to_verdict(decision, tool_name, args, "OTTO.md");
        }
...
fn load_front_matter_rules(project_root: &Path) -> Vec<Rule> {
    let parsed = project::parse_otto_md(project_root);
...
mod tests {
    ...
    async fn add_rule_writes_through_to_disk() {
        ...
        let toml_path = dir.path().join(".otto/permissions.toml");
