...
/// is the `otto-host` crate's `CARGO_PKG_VERSION`, expanded at
...
pub enum AppVersion<'a> {
    ...
    /// `otto-host` crate's `CARGO_PKG_VERSION` and labels the
    /// line `Otto host crate version: <version>` to flag the
...
mod tests {
    ...
    fn build_version_line_uses_host_crate_label_for_fallback() {
        ...
        // otto-host crate's CARGO_PKG_VERSION expands to at compile
        // time. Asserting on the prefix avoids hardcoding the version.
        assert!(s.contains("Otto host crate version:"), "{s}");
    }
