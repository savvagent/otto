...
use otto_plugin::{ContentBlockId, ContentRenderer, PixelFormat, PixelSize};
...
/// Build an `image::DynamicImage` from a plugin-emitted [`otto_plugin::Frame`].
///
/// Frames are RGBA8 by contract (see `crates/otto-canvas/src/canvas.rs`),
...
fn frame_to_dynamic_image(frame: &otto_plugin::Frame) -> Option<image::DynamicImage> {
...
pub enum InputMode {
    ...
    /// [`otto_host::Host::resolve_bash_network_decision`].
    BashNetworkPrompt {
...
pub struct PendingModelChange {
    ...
    pub id: String,
    /// Whether the change should be persisted to `~/.otto/models.toml`.
    pub persist: bool,
}
...
pub enum Entry {
    ...
    Tool {
        ...
        /// step; falls back to `otto_plugin::styled::json_spans` when
...
/// [`otto_host::Host::run_bash_command`].
...
pub struct App {
    ...
    /// buffer lives outside `App` (the egui shell's `OttoApp::prompt`).
    ...
    pub is_loading: bool,
    /// Set when the app handles [`otto_plugin::Effect::Quit`] so the
    ...
    /// `~/.otto/config.toml`'s `[theme]` section at startup;
    ...
    /// from `~/.otto/config.toml`'s `[language]` section (or env
    ...
    /// [`otto_plugin::Effect::RegisterProvider`], keyed by stable
    ...
    pub cached_models: Vec<otto_plugin::ModelEntry>,
    ...
    /// Carries the constructed [`otto_mcp::ProviderClient`] over
    ...
    pub pending_gate: Option<std::sync::Arc<dyn otto_host::PreToolUseGate>>,
    ...
    pub(crate) canvas_click_targets: Vec<(otto_plugin::ContentBlockId, ratatui::layout::Rect)>,
    ...
    pub(crate) html_block_index_to_id: HashMap<u32, otto_plugin::ContentBlockId>,
...
pub(crate) fn canvas_hit(
    targets: &[(otto_plugin::ContentBlockId, ratatui::layout::Rect)],
    col: u16,
    row: u16,
    cell: otto_canvas::CellPixelSize,
) -> Option<(otto_plugin::ContentBlockId, u32, u32)> {
    for (id, rect) in targets {
        let cell_rect = otto_canvas::CellRect {
            col: rect.x,
            row: rect.y,
            width: rect.width,
            height: rect.height,
        };
        if let Some((px, py)) = otto_canvas::cell_to_pixel(cell_rect, cell, col, row) {
...
impl App {
    ...
    #[allow(dead_code)]
    pub(crate) fn is_canvas_focused(&self, id: otto_plugin::ContentBlockId) -> bool {
    ...
    /// [`otto_plugin::ContentBlockId`], and pushes an `Entry::Canvas`
    ...
    pub fn handle_html_block_start(&mut self) -> otto_plugin::ContentBlockId {
        self.flush_live_text();
    ...
    pub fn handle_html_block_stop(&mut self, id: otto_plugin::ContentBlockId) {
        if let Some(Entry::Canvas {
            source,
            source_preview,
            ..
    ...
    pub fn canvas_sources_in_order(&self) -> Vec<(otto_plugin::ContentBlockId, String)> {
        self.entries
            .iter()
                        ...
                        match block {
                            ...
                            ContentBlock::Html { source, state } => {
                                ...
                                let mut renderer: Box<dyn ContentRenderer> =
                                    Box::new(otto_canvas::HtmlCanvas::new(id, source));
    ...
    /// `crates/otto/src/plugin/effects.rs`). This is **not** a
    ...
    pub fn connected_provider_ids(&self) -> Vec<otto_protocol::ProviderId> {
        self.registered_providers
            .keys()
            .filter_map(|s| match otto_protocol::ProviderId::new(s) {
                Ok(id) => Some(id),
                Err(e) => {
                    tracing::warn!(
                        provider_id = %s,
    ...
    /// [`otto_plugin::Effect::PrefillInput`]. The command palette emits
    ...
    pub fn prefill_input(&mut self, text: String) {
        // Bridge for out-of-`App` prompt buffers (egui's `OttoApp::prompt`).
    ...
    /// Persist the active locale to `~/.otto/config.toml`'s
    ...
    /// Persist the active theme to `~/.otto/config.toml`'s
    ...
    pub fn set_active_provider(&mut self, id: otto_plugin::ProviderId) {
    ...
    /// [`otto_mcp::ProviderClient`] in a per-id map and surfaces a note;
    /// PR 7 wires this client into the [`otto_host::Host`] tool-loop.
    pub fn register_provider(
        &mut self,
        id: otto_plugin::ProviderId,
        display_name: String,
        client: Box<dyn otto_mcp::ProviderClient>,
    ) {
        tracing::info!(
    ...
    for item in read_dir.flatten() {
        ...
        let (saved_at, messages) = match &root {
            serde_json::Value::Object(map) if map.contains_key("schema_version") => {
                ...
                let Ok(msgs) =
                    serde_json::from_value::<Vec<otto_protocol::Message>>(msgs_val.clone())
                else {
                    continue;
                };
        ...
        let preview = messages
            .iter()
            .find(|m| m.role == otto_protocol::Role::User)
            .and_then(|m| {
...
mod tests {
    ...
    /// in `otto-host`; this test pins the "what gets pushed" side.
    #[tokio::test]
    ...
    fn bash_network_request_enters_modal_with_id_and_summary() {
        ...
        app.apply_turn_event(TurnEvent::BashNetworkRequested {
            id: 7,
            summary: otto_host::BASH_NETWORK_PROMPT_SUMMARY.into(),
        });
    ...
    fn most_recent_routing_decision_parses_heuristic_badge() {
        // Round-trip pin against `otto_host::router::heuristics::HeuristicKind`'s
    ...
    fn canvas_entry_persists_to_transcript() {
        ...
        app.entries.push(Entry::Canvas {
            id: otto_plugin::ContentBlockId(3),
            source: "<p>x</p>".into(),
            source_preview: None,
        });
    ...
    fn focus_canvas_sets_input_mode_and_is_focused() {
        let mut app = fresh_app();
        let id = otto_plugin::ContentBlockId(7);
    ...
    fn unfocus_canvas_returns_to_editing() {
        let mut app = fresh_app();
        let id = otto_plugin::ContentBlockId(8);
    ...
    fn canvas_hit_maps_cell_to_pixel_offset_in_matching_canvas() {
        let id = otto_plugin::ContentBlockId(3);
    ...
    fn canvas_hit_picks_first_matching_canvas() {
        ...
        let a = otto_plugin::ContentBlockId(1);
        let b = otto_plugin::ContentBlockId(2);
        let targets = vec![
            (
                a,
                ratatui::layout::Rect {
    ...
    /// Auto-export writes a canvas file to `~/.otto/canvases/` after
    ...
    fn transcript_with_assistant_blocks(
        blocks: Vec<otto_protocol::ContentBlock>,
    ) -> TranscriptFile {
        use otto_protocol::{Message, Role};
        TranscriptFile {
            schema_version: 1,
    ...
    fn replay_restores_canvas_state_from_html_block() {
        ...
        let mut state = otto_canvas::CanvasState {
            schema_version: 1,
            ..Default::default()
        };
        ...
        let renderer = app
            .canvas_registry
            .get_mut(otto_plugin::ContentBlockId(0))
            .expect("renderer for id 0");
        let snap = renderer
            .snapshot_state()
            .expect("snapshot non-empty after restore");
        let restored = otto_canvas::CanvasState::from_bytes(&snap).unwrap();
        assert!(
