...
pub(crate) async fn apply_canvas_effects(
    app: &mut App,
    _host_slot: &HostSlot,
    effects: Vec<otto_plugin::Effect>,
) {
    for effect in effects {
        match effect {
            ...
            otto_plugin::Effect::Stack(inner) => {
...
mod tests {
    ...
    struct DispatchStub {
        id: ContentBlockId,
        last_event: Arc<Mutex<Option<otto_plugin::InputEvent>>>,
    ...
    const MISSING_OPENER: &str = "/nonexistent/otto-test-url-opener";
    ...
    async fn raw_key_reaches_renderer_dispatch() {
        ...
        match recorded {
            Some(otto_plugin::InputEvent::Key(recorded_key)) => {
