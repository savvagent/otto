...
use otto_plugin::{ContentBlockId, Frame, PixelFormat, PixelSize};
...
impl GuiCanvasCache {
    ...
    #[allow(dead_code)] // Consumed transitively via OttoApp::clear_canvas_caches.
...
/// [`otto_plugin::MouseEventPortable`] for the painted rect.
///
...
) -> Option<otto_plugin::MouseEventPortable> {
...
/// Map `egui::Modifiers` to the plugin-portable [`otto_plugin::KeyMods`].
///
...
fn modifiers_to_portable(m: &egui::Modifiers) -> otto_plugin::KeyMods {
    otto_plugin::KeyMods {
        ctrl: m.ctrl,
        shift: m.shift,
        alt: m.alt,
...
mod tests {
    ...
    fn mouse_translates_pointer_button_inside_rect() {
        use otto_plugin::{MouseButton, MouseEventKind};
    ...
    fn mouse_pointer_moved_inside_rect_returns_some_move() {
        use otto_plugin::MouseEventKind;
