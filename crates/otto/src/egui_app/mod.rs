...
//! ratatui TUI and launched via the `otto gui` subcommand — see
...
/// What `build_app_with_host` returns — the seed for a [`OttoApp`]. The
...
/// The eframe application. A thin lifecycle wrapper around [`OttoApp`]
...
impl GuiApp {
    ...
    /// only ever touches a live `OttoApp`.
    ///
    ...
    fn poll_boot(&mut self, ctx: &egui::Context) -> Option<&mut OttoApp> {
            ...
            match rx.try_recv() {
                Ok(host) => {
                    ...
                    self.boot = match built {
                        Ok(boot) => {
                            Boot::Running(Box::new(OttoApp::from_boot(boot, self.rt.clone())))
                        }
    ...
    egui::CentralPanel::default().show(ctx, |ui| {
        ui.vertical_centered(|ui| {
            ...
            match error {
                ...
                Some(msg) => {
                    ui.heading("otto — startup failed");
                    ui.add_space(8.0);
