...
//! cached [`RenderModel`] snapshot (produced in `OttoApp::update` before
...
use otto_plugin::manifest::ScreenLayout;
use otto_plugin::{StyledLine, StyledSpan, TextMods};

use super::{FONT_SIZE, OttoApp};
use crate::app::Entry;
...
fn palette_bg(p: &Palette) -> egui::Color32 {
    crate::egui_app::convert::theme_color_to_color32(otto_plugin::ThemeColor::Bg, p)
}
fn palette_border(p: &Palette) -> egui::Color32 {
    crate::egui_app::convert::theme_color_to_color32(otto_plugin::ThemeColor::Border, p)
}

/// Top panel: product name + active provider/model label.
fn paint_header(state: &OttoApp, ctx: &egui::Context) {
...
fn paint_footer(state: &OttoApp, ctx: &egui::Context, palette: &Palette) {
...
fn paint_log(state: &mut OttoApp, ctx: &egui::Context, palette: &Palette) {
    enum EntrySnap {
        User(String),
        Assistant(String),
        Tool(String),
        ...
        egui::ScrollArea::vertical()
            ...
            .show(ui, |ui| {
                ...
                if !state.app.live_text.is_empty() {
                    paint_role_block(ui, palette, "otto", &state.app.live_text);
                }
