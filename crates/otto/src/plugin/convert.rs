...
/// Convert a `otto_plugin::Region` into a `ratatui::layout::Rect`.
...
/// Convert a `ratatui::layout::Rect` into a `otto_plugin::Region`.
...
pub fn theme_color_to_ratatui(c: ThemeColor, palette: &crate::palette::Palette) -> Color {
    match c {
        ...
        // `otto-plugin` that this runtime has not been taught about
...
mod tests {
    ...
    fn key_event_to_portable_null_maps_to_unknown() {
        ...
        let p = super::key_event_to_portable(evt);
        assert!(matches!(p.code, otto_plugin::KeyCodePortable::Unknown));
    }
}
