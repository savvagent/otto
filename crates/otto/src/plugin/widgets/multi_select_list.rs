...
/// [`otto_plugin::Effect`]s. The first consumer is
...
impl<T: Clone> MultiSelectList<T> {
    ...
    pub fn on_key(&mut self, key: otto_plugin::KeyEventPortable) -> MultiSelectOutcome<T> {
...
mod tests {
    ...
    use otto_plugin::{KeyCodePortable as KC, KeyEventPortable, KeyMods};
