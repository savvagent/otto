//! `internal:provider-grok` — keyring-backed xAI Grok shim. Mirrors
//! `provider_deepseek`; see that module for the design notes.
...
impl ProviderGrokPlugin {
    ...
    /// Capability metadata for all Grok models the plugin supports.
    ...
    /// `XAI_API_KEY` environment fallback) and the plugin's static
    /// capability metadata.
    pub(crate) async fn try_build_registration(&self) -> Result<ProviderBuildOutcome, String> {
        ...
        let env_key_present =
            keyring_key.is_none() && std::env::var("XAI_API_KEY").is_ok_and(|v| !v.is_empty());
        if keyring_key.is_none() && !env_key_present {
            return Ok(ProviderBuildOutcome::Unavailable);
        }
        let mut builder = provider_grok::GrokProvider::builder();
        if let Some(key) = &keyring_key {
            builder = builder.api_key(key);
        }
        // else: keyring had no entry but XAI_API_KEY is set — build()
        ...
        .with_aliases(vec![
            ModelAlias {
                alias: "grok".into(),
                provider: ProviderId::new("grok").expect("static alias provider id is valid"),
                model: "grok-4".into(),
            },
            ModelAlias {
                alias: "grok-fast".into(),
                provider: ProviderId::new("grok").expect("static alias provider id is valid"),
    ...
    fn try_connect_from_keyring(&mut self) -> Option<()> {
        ...
        let provider = match provider_grok::GrokProvider::builder().api_key(&key).build() {
            Ok(p) => p,
            Err(e) => {
...
impl Default for ProviderGrokPlugin {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl Plugin for ProviderGrokPlugin {
    fn manifest(&self) -> Manifest {
        ...
        Manifest {
            ...
            description: "xAI Grok provider (Grok 4 family)".into(),
...
impl BuiltinProviderPlugin for ProviderGrokPlugin {
...
mod tests {
    ...
    async fn handle_slash_with_rekey_flag_opens_modal_even_when_client_exists() {
        ...
        let mut p = ProviderGrokPlugin::new();
        let effs = p
            .handle_slash("connect grok", vec!["--rekey".into()])
            .await
            .unwrap();
        assert!(
            effs.iter()
    ...
    fn render_slot_marks_active_provider() {
        ...
        let mut p = ProviderGrokPlugin::with_test_client(Box::new(StubClient));
