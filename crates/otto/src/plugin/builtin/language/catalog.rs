...
//! Persistence lives in `~/.otto/config.toml`'s `[language]` section.
...
/// Load the saved language code from `~/.otto/config.toml`.
///
...
/// Persist `code` to `~/.otto/config.toml`'s `[language]` section.
