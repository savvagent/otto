//! Otto-internal trait that provider plugins implement in addition to
...
//! `otto` crate (NOT in `otto-plugin`) precisely because it
...
/// calls [`take_client`] after observing an [`otto_plugin::Effect::RegisterProvider`]
...
/// should have emitted a [`otto_plugin::Effect::PushNote`] explaining
...
pub(crate) trait BuiltinProviderPlugin: Plugin {
    ...
    /// [`otto_plugin::Effect::RegisterProvider`] from the same plugin.
...
mod caps_helpers_tests {
    use super::*;
    use otto_host::{CostTier, ModelCapabilities};
...
/// after observing [`otto_plugin::Effect::RegisterPreToolGate`]
    ...
    fn builtin_hook_plugin_trait_compiles() {
        ...
        impl otto_plugin::Plugin for Stub {
            fn manifest(&self) -> otto_plugin::Manifest {
                otto_plugin::Manifest {
                    id: otto_plugin::PluginId::new("internal:test-stub").unwrap(),
                    name: "stub".into(),
                    version: "0".into(),
                    description: "stub".into(),
                    kind: otto_plugin::PluginKind::Core,
                    contributions: otto_plugin::Contributions::default(),
                }
            }
        }

        impl super::BuiltinHookPlugin for Stub {
            fn take_pre_tool_gate(&mut self) -> Option<Arc<dyn otto_host::PreToolUseGate>> {
                None
            }
        }
...
pub(crate) mod test_support {
    ...
    mod tests {
        ...
        fn shim_pins_keyring_contract() {
            ...
            // provider-plugin tests' `("otto", PROVIDER_ID)` entries.
            let a = Entry::new("otto-shim-test", "user-a").expect("entry a");
            let b = Entry::new("otto-shim-test", "user-b").expect("entry b");
