        ...
        .await
        {
            ...
            Err(msg) => {
                return Ok(vec![Effect::PushNote {
                    line: otto_plugin::StyledLine::plain(format!("[error] {msg}")),
                }]);
            }
        };
...
mod tests {
    ...
    async fn handle_slash_emits_prompt_send() {
        ...
        assert!(effs.iter().any(|e| matches!(
            e,
            otto_plugin::Effect::PromptSend { text } if text.contains("Hello world")
        )));
    }
    ...
    async fn handle_slash_with_model_emits_override() {
        ...
        assert!(effs.iter().any(|e| matches!(
            e,
            otto_plugin::Effect::SetNextTurnModelOverride { id } if id == "claude-sonnet-4-6"
        )));
    }
    ...
    async fn reload_emits_reindex_and_picks_up_new_files() {
        ...
        assert!(
            effs.iter()
                .any(|e| matches!(e, otto_plugin::Effect::ReindexPlugin { .. }))
        );
    ...
    async fn handle_slash_template_warning_surfaces_as_push_note() {
        ...
        let warn_count = effs
            .iter()
            .filter(|e| matches!(e, otto_plugin::Effect::PushNote { .. }))
            .count();
        let prompt = effs
            .iter()
            .find_map(|e| match e {
                otto_plugin::Effect::PromptSend { text } => Some(text),
                _ => None,
            })
            .unwrap();
    ...
    async fn end_to_end_review_command() {
        ...
        let prompt = effs
            .iter()
            .find_map(|e| match e {
                otto_plugin::Effect::PromptSend { text } => Some(text.as_str()),
                _ => None,
            })
    ...
    async fn user_scoped_command_with_shell_runs_without_trust_modal() {
        ...
        let dir = home.path().join(".otto/commands");
        fs::create_dir_all(&dir).unwrap();
    ...
    async fn handle_slash_session_text_only_with_shell_emits_error_note() {
        ...
        let has_error_note = effs.iter().any(|e| match e {
            otto_plugin::Effect::PushNote { line } => {
    ...
    async fn end_to_end_untrusted_shell_does_not_send_prompt() {
        ...
        assert!(
            effs.iter()
                .any(|e| matches!(e, otto_plugin::Effect::StashPendingSlash { .. }))
        );
        assert!(effs.iter().any(
            |e| matches!(e, otto_plugin::Effect::OpenScreen { id, .. } if id == "trust.modal")
        ));
        assert!(
            !effs
                .iter()
                .any(|e| matches!(e, otto_plugin::Effect::PromptSend { .. }))
        );
    }
}
