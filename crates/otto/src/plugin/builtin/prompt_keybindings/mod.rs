...
/// Undo/redo (otto intercepts + tui-textarea defaults) plus the
...
/// `crates/otto/src/tui.rs`'s `EnableMouseCapture` call, which logs a
...
mod tests {
    ...
    fn dynamic_plugin_rows_become_a_section() {
        ...
        let lines = s.render(otto_plugin::Region {
            x: 0,
            y: 0,
            width: 80,
            height: 200,
