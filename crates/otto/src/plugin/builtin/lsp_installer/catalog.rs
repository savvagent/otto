...
impl Target {
    /// Resolve the current host's target triple, or `None` if otto
...
/// The `[[language]]` entry to merge into `~/.otto/lsp.toml` after
...
pub enum CommandTemplate {
    ...
    /// `~/.otto/lsp-bin/<id>/`. Used by `BinaryDownload` entries
