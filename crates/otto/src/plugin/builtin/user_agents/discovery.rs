...
//! A and B precedence: project beats user, otto beats claude.
...
mod tests {
    ...
    #[test]
    fn precedence_otto_beats_claude_within_project() {
        let project = tempdir().unwrap();
        let user = tempdir().unwrap();
        write_agent(
            &project.path().join(".otto/agents"),
            "shared",
            "---\ndescription: otto version\n---\notto body",
        );
        write_agent(
        ...
        let agents = discover(project.path(), user.path());
        assert_eq!(agents.len(), 1);
        assert!(agents[0].body.contains("otto body"));
    }
    ...
    fn malformed_file_skipped_gracefully() {
        ...
        write_agent(&project.path().join(".otto/agents"), "good", MINIMAL);
        let agents = discover(project.path(), user.path());
    ...
    fn user_agents_discovered_when_no_project() {
        let project = tempdir().unwrap();
        let user = tempdir().unwrap();
        write_agent(&user.path().join(".otto/agents"), "personal", MINIMAL);
        let agents = discover(project.path(), user.path());
        assert_eq!(agents.len(), 1);
