...
//! `otto` binary), this driver replaces every binary in the release
//! archive — `otto` plus the six helper binaries
//! (`otto-anthropic`, `otto-gemini`, `otto-openai`,
//! `otto-tool-fs`, `otto-tool-bash`, `otto-tool-grep`).
//!
//! Mechanism: download the per-release installer (`otto-installer.sh`
//! on Unix, `otto-installer.ps1` on Windows), then exec it through
...
const INSTALLER_FILENAME: &str = "otto-installer.ps1";
#[cfg(not(target_os = "windows"))]
const INSTALLER_FILENAME: &str = "otto-installer.sh";
