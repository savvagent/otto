...
//! v0.11.0 PR 2 (this PR) wires the [`HostStarting`](otto_plugin::HookKind::HostStarting)
...
/// main `otto` binary plus six helpers) by invoking the same
...
const OPT_OUT_ENV_VAR: &str = "OTTO_NO_UPDATE_CHECK";
...
pub struct SelfUpdatePlugin {
    ...
    /// (i.e. `$HOME/.otto/update-check.json`). Tests that exercise
    ...
    /// Re-check cadence. Loaded from `~/.otto/config.toml`'s
...
impl SelfUpdatePlugin {
    ...
    /// Honors `~/.otto/config.toml`'s `[update]` section plus the
    /// `OTTO_NO_UPDATE_CHECK` env var and `--no-update-check` CLI flag.
    ...
    /// without reading the developer's real `~/.otto/config.toml`.
    #[cfg(test)]
    ...
    /// default cadence from `~/.otto/config.toml`'s `[update]` section;
...
mod tests {
    ...
    fn opt_out_cli_flag_disables() {
        assert!(opt_out_from(
            |_| None,
            vec!["otto".to_string(), "--no-update-check".to_string()],
        ));
    }

    #[test]
    fn opt_out_returns_false_when_neither_set() {
        assert!(!opt_out_from(|_| None, vec!["otto".to_string()]));
    }
    ...
    async fn host_starting_auto_installs_on_available_then_writes_cache() {
        ...
        // `~/.otto/update-check.json`, poisoning the next 24h of
