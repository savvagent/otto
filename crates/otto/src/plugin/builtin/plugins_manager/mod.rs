...
fn remove_cmd(args: &[String], home: &Path) -> Result<Vec<Effect>, PluginError> {
    ...
    let candidates: [PathBuf; 2] = [
        home.join(".otto/plugins").join(id),
...
pub(crate) fn summarize_contributions(contributions: &otto_plugin::Contributions) -> String {
...
mod tests {
    ...
    fn summarize_contributions_lists_each_populated_field() {
        ...
        c.slash_commands = vec![otto_plugin::SlashSpec {
            name: "x".into(),
            summary: "".into(),
            args_hint: None,
            requires_arg: false,
        ...
        c.screens = vec![otto_plugin::ScreenSpec {
            id: "x".into(),
