...
//!   `~/.otto/plugins/<id>/`, write a [`TrustFile`] record, and
...
use otto_plugin_wasm::trust::TrustFile;
...
impl PluginsTrustModal {
    ...
    /// Confirm — move staging dir to `~/.otto/plugins/<id>/`, write
    /// trust record, and emit a success note.
    fn confirm(&mut self) -> Vec<Effect> {
        let dest_parent = self.home_dir.join(".otto/plugins");
        ...
        vec![
            Effect::CloseScreen,
            push_note(format!(
                "/plugins install: {} installed; restart otto to load it",
                self.id
            )),
        ]
    }
...
mod tests {
    ...
    async fn confirm_moves_staging_writes_trust_and_closes() {
        ...
        let installed_dir = home.join(".otto/plugins/acme.demo");
    ...
    async fn cancel_removes_staging_and_writes_no_trust() {
        ...
        assert!(
            !home.join(".otto/plugins/acme.demo").exists(),
    ...
    async fn confirm_rejects_existing_install() {
        ...
        std::fs::create_dir_all(home.join(".otto/plugins/acme.demo")).unwrap();
