...
/// On-disk shape of `~/.otto/plugins.toml`. Internal to the
...
/// Compute the path to `~/.otto/plugins.toml`. Returns `None` if
...
/// developer's real `~/.otto/plugins.toml`.
pub fn config_path() -> Option<PathBuf> {
    let raw = std::env::var_os("HOME")?;
    if raw.is_empty() {
        return None;
    }
    Some(PathBuf::from(raw).join(".otto").join("plugins.toml"))
}
