...
impl Plugin for UserHooksPlugin {
    fn manifest(&self) -> Manifest {
        ...
        contributions.hooks = vec![
            ...
            otto_plugin::HookKind::PromptSubmitted, // -> UserPromptSubmit
            otto_plugin::HookKind::TurnStart,       // resets prev_stop_blocked
            otto_plugin::HookKind::TurnEnd,         // -> Stop
            otto_plugin::HookKind::SubagentStop,    // -> SubagentStop
        ];
        Manifest {
...
mod tests {
    ...
    async fn reload_hooks_through_app_index_round_trip() {
        ...
        // dev machine's real `~/.otto/settings.json`.
        let _lock = HOME_LOCK.lock().unwrap();
        let _home_guard = HomeGuard::new();

        let proj = TempDir::new().unwrap();
        std::fs::create_dir_all(proj.path().join(".otto")).unwrap();
        std::fs::write(
            proj.path().join(".otto/settings.json"),
            r#"{
                "hooks": {
                    "PreToolUse": [
                        {
        ...
        // `project_root.join(".otto")` first, independent of the home
    ...
    async fn e2e_pre_tool_use_block_short_circuits() {
        ...
        // pick up hooks from the dev machine's real ~/.otto/settings.json.
        let _lock = HOME_LOCK.lock().unwrap();
        let _home_guard = HomeGuard::new();
    ...
    /// `otto` crate is binary-only — there is no `lib.rs` to
