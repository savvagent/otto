...
impl ProviderDeepSeekPlugin {
    ...
    /// `DEEPSEEK_API_KEY` environment fallback) and the plugin's static
    /// capability metadata.
    pub(crate) async fn try_build_registration(&self) -> Result<ProviderBuildOutcome, String> {
        ...
        let env_key_present =
            keyring_key.is_none() && std::env::var("DEEPSEEK_API_KEY").is_ok_and(|v| !v.is_empty());
        if keyring_key.is_none() && !env_key_present {
            return Ok(ProviderBuildOutcome::Unavailable);
        }
        let mut builder = provider_deepseek::DeepSeekProvider::builder();
        if let Some(key) = &keyring_key {
            builder = builder.api_key(key);
        }
        // else: keyring had no entry but DEEPSEEK_API_KEY is set — build()
...
mod tests {
    ...
    /// The picker dispatches `connect deepseek` with a stored key — this must NOT emit
    ...
    /// The picker dispatches `connect deepseek` with a re-key (Alt+Enter) —
