...
/// Persists per-user state in `~/.otto/plugins.toml`.
...
/// Routes turns via `~/.otto/routing.toml`; registered as `/route [reload | show]`.
pub mod route;
...
/// Otto-internal [`provider_common::BuiltinProviderPlugin`] trait —
...
/// xAI Grok provider shim: keyring-backed `internal:provider-grok`.
pub mod provider_grok;
...
/// a static canvas via otto-canvas / Blitz. Contributes the
