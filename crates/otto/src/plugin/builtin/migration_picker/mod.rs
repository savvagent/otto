...
mod tests {
    ...
    // HOME_LOCK is std::sync::Mutex (shared across the otto crate)
    ...
    // raced these tests' writes to `~/.otto/config.toml`.
    ...
    async fn on_event_ignores_non_host_starting() {
        ...
        let effs = p
            .on_event(HostEvent::ProviderRegistered {
                id: otto_plugin::ProviderId::new("anthropic").unwrap(),
