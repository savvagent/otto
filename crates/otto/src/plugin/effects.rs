...
use otto_plugin::{Effect, HostEvent, PluginId, PluginKind, ScreenArgs, UrlTarget};
...
/// `~/.otto/plugins.toml`. Errors here are returned to the caller so
...
/// Format a [`otto_plugin::ChordPortable`] for the keybindings
/// viewer. Modifier order matches the conventional `Ctrl+Alt+Shift+X`
/// display so the rendered chords look familiar.
fn format_chord(chord: &otto_plugin::ChordPortable) -> String {
...
/// [`otto_plugin::HookKind`], then apply each subscriber's returned
...
///    existing [`otto_host::TurnEvent`] stream.
...
mod tests {
    ...
    /// developer's real `~/.otto/`. Tokio's `current_thread` flavor
    ...
    async fn toggle_plugin_optional_updates_registry_and_persists() {
        ...
        use crate::plugin::registry::{BuiltinSet, PluginRegistry};
        use async_trait::async_trait;
        use otto_plugin::{Contributions, Manifest, Plugin, PluginId, PluginKind};
        ...
        // And ~/.otto/plugins.toml carries the override.
    ...
    async fn toggle_plugin_rolls_back_on_indexes_build_failure() {
        ...
        use crate::plugin::registry::{BuiltinSet, PluginRegistry};
        use async_trait::async_trait;
        use otto_plugin::{Contributions, Manifest, Plugin, PluginId, PluginKind, SlashSpec};
    ...
    async fn set_active_locale_persist_true_switches_rust_i18n_and_writes_file() {
        ...
        apply_effects(
            &mut app,
            vec![otto_plugin::Effect::SetActiveLocale {
                code: "es".into(),
                persist: true,
            }],
        )
    ...
    async fn connect_picker_lists_all_provider_plugins() {
        ...
        let ids: std::collections::HashSet<String> = candidates
            ...
            .collect();
        for expected in ["anthropic", "gemini", "openai", "deepseek", "grok", "local"] {
            assert!(
    ...
    /// touches the developer's real `~/.otto/`.
    ///
    ...
    ///
    /// We make `~/.otto/trusted-projects.json` unwritable by pre-creating
    ...
    async fn set_trust_level_always_save_failure_emits_warn_note() {
        ...
        // Pre-create ~/.otto/trusted-projects.json as a *directory* so
        // that the subsequent fs::write inside trust::save will fail.
        let home = dirs::home_dir().expect("HomeGuard must set HOME");
        let otto_dir = home.join(".otto");
        std::fs::create_dir_all(&otto_dir).expect("create .otto dir");
        let file_path = otto_dir.join("trusted-projects.json");
    ...
    async fn app_new_loads_persisted_trust_from_disk() {
        ...
        let (app, expected_key) = {
            ...
            let home = dirs::home_dir().expect("HomeGuard must set HOME");
            let otto_dir = home.join(".otto");
            std::fs::create_dir_all(&otto_dir).unwrap();
    ...
    async fn register_pre_tool_gate_no_hook_entry_warns() {
        ...
        let pid = otto_plugin::PluginId::new("internal:does-not-exist").unwrap();
        let effs = vec![otto_plugin::Effect::RegisterPreToolGate { plugin_id: pid }];
