...
/// Conversion helpers between `otto-plugin` types and ratatui types.
pub mod convert;
...
/// Keybinding routing: resolves a portable key event to its [`otto_plugin::BoundAction`]
...
/// Sequential awaited dispatch of [`otto_plugin::HostEvent`]s to subscribed
...
/// `otto-plugin-wasm` crate; the otto crate composes built-ins
...
pub(crate) fn register_builtins(
    ...
    mcp_statuses: Vec<otto_host::ToolServerStatus>,
) -> BuiltinSet {
...
mod tests {
    ...
    async fn html_renderer_resolves_through_registry_and_indexes() {
        ...
        let r = guard.create_renderer("html", otto_plugin::ContentBlockId(0), "<p>x</p>");
        assert!(
            r.is_ok(),
