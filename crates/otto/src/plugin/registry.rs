...
impl PluginRegistry {
    ...
    pub fn active_prompt_segments(&self) -> Vec<otto_plugin::SystemPromptSegment> {
        let mut out = Vec::new();
    ...
    /// observing [`otto_plugin::Effect::RegisterProvider`].
    ///
...
impl Plugin for BoxedPlugin {
    ...
    async fn handle_slash(
        ...
        name: &str,
        args: Vec<String>,
    ) -> Result<Vec<otto_plugin::Effect>, otto_plugin::PluginError> {
    ...
    ) -> Result<Box<dyn otto_plugin::Screen>, otto_plugin::PluginError> {
    ...
    ) -> Option<Vec<otto_plugin::StyledSpan>> {
    ...
    fn themes(&self) -> Vec<otto_plugin::ThemeEntry> {
        self.0.themes()
    }
    ...
    fn create_renderer(
        ...
        source: &str,
    ) -> Result<Box<dyn otto_plugin::ContentRenderer>, otto_plugin::PluginError> {
...
mod hook_entry_tests {
    use super::*;
    use async_trait::async_trait;
    use otto_plugin::{Contributions, Manifest, PluginId, PluginKind};
