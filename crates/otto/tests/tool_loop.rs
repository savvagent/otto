...
//! - The real `otto-tool-fs` binary (built from this crate) is spawned
//!   as a child stdio MCP server pointed at a temp directory. Cargo sets
//!   `CARGO_BIN_EXE_otto-tool-fs` for tests in the same package as the
...
use otto_host::{Host, HostConfig, ProviderEndpoint, ToolCallStatus, ToolEndpoint};
...
fn tool_fs_bin() -> PathBuf {
    PathBuf::from(env!("CARGO_BIN_EXE_otto-tool-fs"))
}
