...
//! manifest id.
//!
//! ## Why this lives in the otto crate's tests/, not plugin-wasm's
//!
...
//! `crates/otto-plugin-wasm/tests/fixtures/static.wasm`. If the
...
async fn trusted_static_plugin_appears_in_registered_plugins() {
    ...
    // Stage a plugin under the user-scope `.otto/plugins/` tier.
    ...
    let plugin_dir = home.join(".otto/plugins").join(plugin_id);
...
"#,
    ...
    // `otto_plugin_wasm::convert::disk_id_to_plugin_id`.
