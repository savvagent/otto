//! Acceptance tests for `DeepSeekMcpServer`.
//!
//! Each test stands up a fake DeepSeek HTTP server, builds the SPP MCP server
//! around an [`DeepSeekProvider`] pointed at it, and drives it through an
...
use provider_deepseek::{DeepSeekMcpServer, provider_for_tests};
use rmcp::{
...
async fn spawn_fake_deepseek(mode: FakeMode) -> String {
    let app = Router::new()
...
async fn spawn_mcp_server(deepseek_base: &str) -> String {
    let provider = provider_for_tests(deepseek_base.to_string());
    let provider = Arc::new(provider);
    let provider_for_factory = provider.clone();
    let svc: StreamableHttpService<DeepSeekMcpServer, LocalSessionManager> =
...
fn req_text(prompt: &str) -> serde_json::Map<String, serde_json::Value> {
    let req = CompleteRequest::text("deepseek-v4-flash", prompt, 16);
...
async fn non_streaming_complete_round_trips() {
    ...
    let upstream = spawn_fake_deepseek(FakeMode::Json(canned)).await;
...
const FROZEN_SSE: &str = concat!(
    "data: {\"id\":\"chatcmpl-frozen\",\"model\":\"deepseek-v4-flash\",\"choices\":[{\"delta\":{\"role\":\"assistant\",\"content\":\"\"},\"finish_reason\":null}]}\n\n",
    "data: {\"id\":\"chatcmpl-frozen\",\"model\":\"deepseek-v4-flash\",\"choices\":[{\"delta\":{\"content\":\"hello\"},\"finish_reason\":null}]}\n\n",
    "data: {\"id\":\"chatcmpl-frozen\",\"model\":\"deepseek-v4-flash\",\"choices\":[{\"delta\":{\"content\":\" world\"},\"finish_reason\":null}]}\n\n",
    "data: {\"id\":\"chatcmpl-frozen\",\"model\":\"deepseek-v4-flash\",\"choices\":[{\"delta\":{},\"finish_reason\":\"stop\"}],\"usage\":{\"prompt_tokens\":7,\"completion_tokens\":2,\"total_tokens\":9}}\n\n",
    "data: [DONE]\n\n",
);
...
async fn streaming_complete_emits_progress_and_final_response() {
    let upstream = spawn_fake_deepseek(FakeMode::Sse(FROZEN_SSE)).await;
    let mcp_url = spawn_mcp_server(&upstream).await;
...
async fn complete_tool_is_advertised() {
    let upstream = spawn_fake_deepseek(FakeMode::Json(json!({}))).await;
    let mcp_url = spawn_mcp_server(&upstream).await;
    let (client, _rx) = make_client();
    let transport = StreamableHttpClientTransport::from_config(
        StreamableHttpClientTransportConfig::with_uri(mcp_url),
