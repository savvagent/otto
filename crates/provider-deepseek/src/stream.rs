//! DeepSeek SSE → SPP [`StreamEvent`] adapter.
//!
//! DeepSeek's Chat Completions streaming endpoint emits `data:` SSE lines whose
...
use crate::api;
use crate::translate::{parse_tool_arguments, stop_reason_from_str, usage_from_deepseek};

/// Drive a DeepSeek SSE streaming response to completion.
///
...
struct Accumulator {
    ...
    /// Per-DeepSeek-tool-call-index → SPP block index mapping.
...
impl Accumulator {
    fn consume_chunk(&mut self, chunk: api::ChatCompletionChunk) -> Vec<StreamEvent> {
        ...
        if let Some(u) = chunk.usage {
            let spp_usage = usage_from_deepseek(u);
    ...
    fn flush(&mut self) -> Vec<StreamEvent> {
        ...
        if finish.unclosed_fence {
            tracing::warn!("deepseek stream ended with unclosed html-canvas fence");
        }
        ...
        // own Stop event mid-stream — DeepSeek streams them as a series of
...
mod tests {
    ...
    async fn sse_decoder_errors_on_truncated_trailing_frame() {
        ...
        let bytes = bytes::Bytes::from_static(
            b"data: {\"id\":\"c1\",\"model\":\"deepseek-v4-flash\",\"choices\":[{\"delta\":{\"content\":\"hi\"},\"finish_reason\":null}]}\n\ndata: {\"id\":\"c1\",\"model\":\"deepseek-v4-flash\""
        );
