//! Pure translation between SPP and DeepSeek Chat Completions types.
//!
//! DeepSeek differs from SPP in a few structural ways this module hides:
//!
//! 1. **System messages** are first-class role-bearing messages in DeepSeek's
//!    schema, not a separate top-level field.
//! 2. **Tool-call identity.** SPP carries a `tool_use_id` linking each
//!    `ToolUse` to its matching `ToolResult`; DeepSeek uses the same `id`
...
//!    DeepSeek requires a dedicated `tool` role message with a `tool_call_id`.
...
/// Translate an SPP request into a DeepSeek Chat Completions body.
pub fn request_to_deepseek(req: &spp::CompleteRequest, stream: bool) -> api::ChatCompletionRequest {
    ...
    let tools: Vec<api::Tool> = req.tools.iter().map(tool_to_deepseek).collect();
    // Intentionally leave `tool_choice` unset. DeepSeek defaults to `"auto"`
...
/// Translate a non-streaming DeepSeek response into an SPP [`spp::CompleteResponse`].
pub fn response_from_deepseek(r: api::ChatCompletionResponse) -> spp::CompleteResponse {
    let usage = r.usage.map(usage_from_deepseek).unwrap_or_default();
                    ...
                    spp::ContentBlock::ToolUse { id, name, input } => {
                        tool_calls.push(api::RequestToolCall {
                            ...
                            function: api::RequestFunction {
                                name: name.clone(),
                                // DeepSeek expects arguments as a JSON string.
            ...
            for b in &m.content {
                match b {
                    ...
                    spp::ContentBlock::Image { source } => {
                        if let Some(part) = image_to_deepseek(source) {
...
fn image_to_deepseek(source: &spp::ImageSource) -> Option<api::ContentPart> {
...
fn tool_to_deepseek(t: &spp::ToolDef) -> api::Tool {
    api::Tool {
...
pub(crate) fn usage_from_deepseek(u: api::UsageStats) -> spp::Usage {
    spp::Usage {
...
mod tests {
    ...
    fn translates_simple_text_request() {
        let req = spp::CompleteRequest::text("deepseek-v4-flash", "hello", 64);
        let body = request_to_deepseek(&req, false);
    ...
    fn parses_minimal_non_streaming_response() {
        ...
        let resp = response_from_deepseek(r);
