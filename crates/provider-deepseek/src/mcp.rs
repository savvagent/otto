//! MCP server wrapper around [`DeepSeekProvider`].
//!
...
/// MCP server that exposes [`DeepSeekProvider`] as an SPP-conformant
...
impl std::fmt::Debug for DeepSeekMcpServer {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("DeepSeekMcpServer").finish_non_exhaustive()
    }
}

impl DeepSeekMcpServer {
    /// Wrap a provider for service over MCP.
    pub fn new(provider: DeepSeekProvider) -> Self {
        Self {
    ...
    pub fn from_shared(provider: Arc<DeepSeekProvider>) -> Self {
        Self {
            provider,
            tool_router: Self::tool_router(),
        }
...
impl DeepSeekMcpServer {
    ...
    #[tool(
        name = "complete",
        description = "Run a completion against DeepSeek's Chat Completions API."
    )]
    ...
    /// SPP `list_models` tool. Queries DeepSeek's `/models` endpoint and
    /// returns every listed model — DeepSeek's catalog has no non-chat
            ...
            .with_server_info(
                Implementation::new(env!("CARGO_PKG_NAME"), env!("CARGO_PKG_VERSION"))
                    .with_description(format!(
                        "Otto DeepSeek provider — SPP {} via the `{}` tool",
                        spp::SPP_VERSION,
                        COMPLETE_TOOL_NAME
                    )),
            )
            .with_instructions(format!(
                "SPP-conformant DeepSeek provider. Call `{}` with a CompleteRequest. \
...
mod mcp_tests {
    use super::*;
    use crate::{DEFAULT_MODEL, DeepSeekProvider};
    use axum::{Json, Router, routing::get};
    use otto_protocol::ListModelsResponse;
    use serde_json::json;

    /// `list_models_tool` is the MCP wrapper around `DeepSeekProvider::list_models`.
    ...
    /// id present (DeepSeek's catalog has no non-chat clutter to filter).
    #[tokio::test]
    async fn list_models_tool_returns_structured_set() {
        ...
        let provider = DeepSeekProvider::builder()
            ...
            .build()
            .unwrap();
        let server = DeepSeekMcpServer::new(provider);
