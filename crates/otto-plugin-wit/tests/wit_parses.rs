...
//! resolve in a single `otto:plugin@0.1.0` package, with each world
...
fn package_namespace_and_interfaces() {
    ...
    assert_eq!(
        pkg.name.namespace, "otto",
        "package namespace must stay `otto`",
    );
    assert_eq!(
...
fn plugin_interactive_world_shape() {
    ...
    assert_world_shape(
        ...
        &["manifest", "otto:plugin/screens@0.1.0"],
        &["log", "current-theme"],
    );
}
...
fn plugin_provider_world_shape() {
    ...
    assert_world_shape(
        ...
        &[
            "log",
            "otto:plugin/http-capability@0.1.0",
            "otto:plugin/keyring-capability@0.1.0",
            "otto:plugin/progress-capability@0.1.0",
        ],
    );
}
