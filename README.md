...
| `~/.savvagent/config.toml` | TUI startup, `/theme`, `/language`, `internal:self-update` | Startup connection policy (`opt-in` / `all` / `last-used` / `none`), `startup_providers` list, per-provider `connect_timeout_ms`, one-time migration flag, `[language].code`, `[theme].name`, and `[update]` settings (`periodic_interval_secs`, `disabled`). Created automatically on first launch when multiple keyring entries are found, and updated when those features persist settings. |
