...
discovery pattern (`.otto/` and `.claude/` × project and user scopes).
...
fork otto or use the model itself as the policy layer (unreliable).
...
— a new `PreToolUseGate` trait in `otto-host` is consulted by
...
startup via a otto-internal effect (same pattern providers use).
...
Sequential execution within an event respects this order: project-otto
hooks fire first, then project-claude, then user-otto, then user-claude.
...
"Project root" is the same root the `OTTO.md` loader resolves to (walk
up from cwd looking for `.git/`, `.otto/`, or root).
...
Claude Code event name → otto internal `HookKind`:
...
- `OTTO_PROJECT_DIR` — same path the stdin's `cwd` carries; convenience for shell scripts that want it as `$OTTO_PROJECT_DIR`.
...
```
crates/otto/src/plugin/builtin/user_hooks/
    ...
    pre_tool_gate.rs      # impl PreToolUseGate (otto-host trait)
...
synchronous gating before tool dispatch. Add a otto-internal trait
to `otto-host` (NOT the WIT-portable surface, same pattern as
`BuiltinProviderPlugin` for providers):

```rust
// crates/otto-host/src/pre_tool_gate.rs (new)
...
A new otto-internal effect (NOT WIT-portable — it traffics a
`PluginId`, then the runtime resolves the trait object via the
registry):

```rust
// in crates/otto-plugin/src/effect.rs
/// Announce that this plugin provides a `PreToolUseGate`. The runtime
/// fetches the gate object via a otto-internal seam (not part of
...
`BuiltinHookPlugin` is the otto-internal trait the user-hooks
plugin implements (in `crates/otto/src/plugin/builtin/`):
...
- **`Notification` event** — otto has no analog. Reserved.
- **Hook priority / dependency ordering** — sequential in config-merge order is the only model. No `priority` field.
- **Per-hook env overrides** — hooks inherit TUI env plus `OTTO_PROJECT_DIR`. No knobs.
...
- Update README: new "User-defined hooks" section under TUI features; add
  `.otto/settings.json` and `~/.otto/settings.json` to the
...
| Test | Crate | File |
|------|-------|------|
| Discovery walks all four paths; merges hook lists in precedence order | `otto` | `user_hooks/discovery.rs` |
| Malformed JSON warn-and-skip; other files still load | `otto` | `user_hooks/discovery.rs` |
| Unknown event names (`Notification`, `SubagentStop`) warn once per file, parse rest | `otto` | `user_hooks/config.rs` |
| Matcher: `*` / exact / `tool-fs:*` / suffix glob | `otto` | `user_hooks/matcher.rs` |
| Matcher: invalid pattern warn-and-skip | `otto` | `user_hooks/matcher.rs` |
| Payload round-trip per event type | `otto` | `user_hooks/payload.rs` |
| Runner: exit 0 / exit 2 (block) / exit 127 / non-zero non-2 / timeout | `otto` | `user_hooks/runner.rs` |
| Runner: structured JSON stdout takes precedence; invalid JSON falls back | `otto` | `user_hooks/runner.rs` |
| Runner: `hookEventName` mismatch warn-logs and ignores `hookSpecificOutput` | `otto` | `user_hooks/runner.rs` |
| `PreToolUseGate` Allow → tool runs | `otto-host` | `pre_tool_gate.rs` |
| `PreToolUseGate` Block → `ToolCallOutcome::error` with reason | `otto-host` | `pre_tool_gate.rs` |
| `PreToolUseGate` panic → fail-open with log | `otto-host` | `pre_tool_gate.rs` |
| `PreToolUseGate` Block also fires `ToolCallEnd` with `success: false` | `otto-host` | session integration |
| `UserPromptSubmit` hook with `additionalContext` → `PrependToPendingPrompt` effect | `otto` | `user_hooks/mod.rs` |
| `UserPromptSubmit` hook exit-2 → `CancelPendingTurn` effect | `otto` | `user_hooks/mod.rs` |
| `Stop` hook block → `CancelPendingTurn`; next turn does NOT auto-fire | `otto` | `user_hooks/mod.rs` |
| `/reload-hooks` replaces index without restart | `otto` | `user_hooks/reload.rs` |
| Concurrent `/reload-hooks` during in-flight `PreToolUse` — in-flight uses old gate snapshot | `otto` | `user_hooks/reload.rs` |
| End-to-end: `PreToolUse` hook on `tool-fs:write_file` denies a write; tool result is the deny reason | `otto` | `tests/user_hooks_e2e.rs` |
| `SubagentStop` parsed cleanly, never fires | `otto` | `user_hooks/config.rs` |
...
1. `globset` may not be a direct dep of `otto` even if it's transitively
   present via `ignore`. Plan inspects `crates/otto/Cargo.toml` and
    ...
    "PreToolUse": [
      {
        "matcher": "tool-fs:write_file",
        "hooks": [
          { "type": "command", "command": "~/bin/otto-write-check.sh" }
        ]
      },
      {
        "matcher": "run",
        "hooks": [
          { "type": "command", "command": "~/bin/otto-bash-allowlist.sh", "timeout": 5 }
        ]
      }
    ],
    "UserPromptSubmit": [
      {
        "matcher": "*",
        "hooks": [
          { "type": "command", "command": "~/bin/otto-secrets-scan.sh" }
        ]
      }
    ],
    "Stop": [
      {
        "matcher": "*",
        "hooks": [
          { "type": "command", "command": "~/bin/otto-pr-checklist.sh" }
        ]
      }
    ]
  }
...
Where `otto-write-check.sh` might be:
