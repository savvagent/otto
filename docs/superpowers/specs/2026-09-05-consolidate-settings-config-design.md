...
`savvagent` currently stores user-facing settings across three unrelated surfaces:

- `~/.savvagent/config.toml` already holds sectioned startup and migration state via `ConfigFile` (`crates/savvagent/src/config_file.rs:31-104`).
- Language selection persists to a dedicated `~/.savvagent/language.toml` via ad hoc helpers in `crates/savvagent/src/plugin/builtin/language/catalog.rs:78-154`.
- Theme selection persists to a dedicated `~/.savvagent/theme.toml` via parallel helpers in `crates/savvagent/src/plugin/builtin/themes/catalog.rs:224-271`.
- Self-update behavior mixes configuration sources: `SAVVAGENT_NO_UPDATE_CHECK` still controls the disable path and a hardcoded `PERIODIC_INTERVAL` of two hours drives the periodic loop in `crates/savvagent/src/plugin/builtin/self_update/mod.rs:76-92` and `:138-227`.
...
Issue #21 asks to consolidate these knobs into `~/.savvagent/config.toml`, explicitly removing the old per-feature TOML files with no back-compat/migration because the project has no users yet.
...
1. **Extend `ConfigFile` in place.** Add `LanguageSection`, `ThemeSection`, and `UpdateSection` to `crates/savvagent/src/config_file.rs`, each behind `#[serde(default)]` so missing sections remain additive for existing `config.toml` readers. Mirror the file’s existing style: plain structs, explicit default helpers where needed, one `load_or_default` entry point, and focused unit tests in the same file.
2. **Route language persistence through `ConfigFile`.** Replace `LanguageConfig`, `config_path()`, `load()`, and `save()` in `crates/savvagent/src/plugin/builtin/language/catalog.rs` with helpers that read/write `ConfigFile::default_path()`. Keep locale detection precedence (`saved config` → `LC_ALL` → `LC_MESSAGES` → `LANG` → `en`) and unsupported-code fallback semantics, but make the saved source `config.toml` instead of `language.toml`.
...
**In:**
- `crates/savvagent/src/config_file.rs`
- `crates/savvagent/src/plugin/builtin/language/catalog.rs`
- `crates/savvagent/src/plugin/builtin/themes/catalog.rs`
...
- Removing the `SAVVAGENT_NO_UPDATE_CHECK` env var or `--no-update-check` CLI flag
- Any provider/host/tool transport changes, plugin ABI changes, or TUI host-swap behavior changes

## Public-interface changes

**Breaking on-disk configuration change.** This change adds new optional sections to `~/.savvagent/config.toml` (additive within that file), but it also **removes** the previously-documented standalone `~/.savvagent/language.toml` and `~/.savvagent/theme.toml` persistence files. Per the workflow’s Non-Negotiable Rule 6, removing those files is a deliberate public-interface change to the on-disk configuration surface and must be:
...
The update configuration change is otherwise additive: `[update]` introduces file-backed defaults while retaining `SAVVAGENT_NO_UPDATE_CHECK` / `--no-update-check` as higher-precedence overrides.
...
1. **`ConfigFile` already has the right persistence primitives.** The issue’s requested `LanguageSection` / `ThemeSection` / `UpdateSection` additions fit the existing `load_or_default` + `save` model in `crates/savvagent/src/config_file.rs:78-103`; no new config framework or migration subsystem is needed.
