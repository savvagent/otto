...
- Startup can seed the banner from cached "newer version available" state immediately, but it still revalidates with GitHub before deciding which release to install.
...
1. **Tighten first-tick cache trust.** Keep the existing cache file and TTL, but change the first-tick decision in `crates/otto/src/plugin/builtin/self_update/mod.rs` so cached data is never treated as sufficient proof that the running binary is current. A cached `Equal` result becomes a "must revalidate now" case instead of a startup skip, because it cannot prove that GitHub has not published a newer release since the cache write. A cached `Behind` result still seeds the banner immediately, but the same startup tick must revalidate with GitHub before choosing which release to install.
...
- **The 24h cache should remain as a UX optimization, not a startup network-skip authority.** Rationale: the issue asks for correctness, and cached data is still useful for seeding the banner instantly even though launch now revalidates live.
...
- If cached data shows a newer release (`Behind`), use it to surface an immediate banner state, then revalidate live before deciding which release to install.
...
- **Risk: more startup GitHub requests on every launch.** This is intentional; correctness on startup now outweighs the old cache-skip optimization, and the cache is retained only to seed immediate user-visible state.
