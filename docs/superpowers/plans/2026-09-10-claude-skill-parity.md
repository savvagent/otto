...
`.github/skills/**/*.md` — the canonical bodies — are edited only where the canonical itself has a bug, and every such edit is mirrored into the port with the record regenerated in the same commit. That is the *normal* workflow this plan installs, not scope creep: `d4824da` fixed a `\` line continuation followed by a `#` comment (broken bash) in `creating-github-issues/SKILL.md`, mirrored it, regenerated the record, and the check stayed green — the round trip's worked example. What is forbidden is editing a **port** in place: a port is regenerated from its canonical, so an in-place edit is lost work and is exactly the drift the check catches. Otherwise only new `claude-port/` subdirectories are added beside the canonicals. `.claude/skills/rust-engineer/` and `tui-engineer/` are not touched: Claude-Code-native, no canonical counterpart, not iterated by the check.
...
- [x] **Step 1: Every canonical edit is mirrored.** `git diff --name-only origin/main...HEAD -- .github/skills/` lists the `claude-port/*.diff` records plus any canonical `.md` this branch legitimately fixed. A canonical `.md` in that list is **not** scope creep by itself — editing the canonical is the first half of the workflow. What it obliges: for each such file, confirm the port under `.claude/skills/` carries the same edit and its record was regenerated, then confirm `bash .github/scripts/check-claude-skill-ports.sh` exits 0. On this branch that is `creating-github-issues/SKILL.md` via `d4824da` (a `\` line continuation followed by a `#` comment — broken bash), mirrored and re-recorded. What *would* be scope creep is a canonical edit that changes the workflow rather than fixing a defect in it, or any edit made directly to a **port**.
...
- [x] **Step 8: Record what the PR body must state.**
  ...
  3. **The honest cost**: this is a second copy of a ~103KB document. Say so, and say how it is contained (~8% divergence — 132 of 1,602 canonical lines — recorded exactly, CI-enforced, loud failure) and what would remove it (making the canonical bodies host-neutral — the natural follow-up, out of scope here).
