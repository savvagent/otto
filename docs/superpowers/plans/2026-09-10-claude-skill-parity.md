...
| `.github/scripts/check-claude-skill-ports.sh` (new) | Recomputes each `diff canonical ported` and compares to the record; asserts frontmatter parity separately; fails on missing/orphaned ports and records, and on an empty canonical root. First `.sh` in the repo. |
| `.gitattributes` (new) | `*.sh text eol=lf` — keeps the script's shebang LF on a `core.autocrlf=true` clone. |
| `.github/workflows/ci.yml` (modify) | One step in the existing `lint` job, immediately after `actions/checkout@v4`. |
...
- [x] **Step 1: `.gitattributes`** — `*.sh text eol=lf`, with a comment saying why (a CRLF shebang fails confusingly).
...
- [x] **Step 2: Write the check.** Requirements:
  ...
  - For each directory under `.github/skills/`: every canonical `*.md` must have a port at `.claude/skills/<name>/<basename>` and a record at `.github/skills/<name>/claude-port/<basename>.diff`; an orphaned record fails too.
  - Recompute the diff through **one** `port_diff()` function used for both generation and verification, so the two forms cannot diverge. Use `diff -u -L <canonical> -L <ported>` — plain `diff -u` emits mtimes, which would make every record machine- and date-specific.
  - Compare byte-for-byte against the record. On mismatch, print the two commands to fix it and a diff-of-diffs.
  - Assert `name`/`description` parity **separately**, so the failure that changes trigger behaviour is reported as itself rather than as an opaque diff mismatch. Read only the leading `---` fenced block, strip trailing `\r`, strip one optional layer of wrapping quotes, fail loudly on a missing or empty key.
...
**Files:** none permanently — every mutation reverted.
...
- [x] **Step 7:** Confirm the tree is clean (`git status --porcelain` scoped to the mutated paths) and the check is back to `exit=0`.
...
- [ ] **Step 1: Canonical bodies untouched.** `git diff --name-only origin/main...HEAD -- .github/skills/` must list **only** `claude-port/*.diff` additions. A canonical `.md` in that list means scope crept.
- [ ] **Step 2: Changed-file set matches the File Structure table**, via `git diff --name-status origin/main...HEAD`. No Rust file, no `Cargo.toml`, no `Cargo.lock`, no `CHANGELOG.md`.
...
- [ ] **Step 4: Public-interface record.** Nothing changed: no SPP wire type, no `ProviderHandler`/`ProviderClient` method, no tool MCP schema, no plugin ABI surface, no slash command, no env var, no on-disk transcript/keyring format. Non-Negotiable Rule 6 is not engaged; no `CHANGELOG.md` interface note is owed by this PR.
- [ ] **Step 5: Rust invariants — vacuously satisfied, stated explicitly.** No `crates/otto/src/app.rs` or `tui.rs` edit, so the host-swap `RwLock` rule is not engaged; no streaming provider path, so the `ProgressDispatcher` forwarder-abort pattern is not engaged; no crate boundary or dependency edge changes. Verify from Step 2's file list, not by assertion.
...
- [ ] **Step 8: Record what the PR body must state.**
  ...
  3. **The honest cost**: this is a second copy of a ~103KB document. Say so, and say how it is contained (~4% divergence, recorded exactly, CI-enforced, loud failure) and what would remove it (making the canonical bodies host-neutral — the natural follow-up, out of scope here).
