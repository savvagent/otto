...
- [x] Open the PR against `main` with a body referencing `savvagent/otto#83`, and run the three required review passes (Rust-expert, architecture, and an independent diff-only `security-review` that receives **only** the PR diff — no spec, no plan, no summary). Shipped as [#153](https://github.com/savvagent/otto/pull/153), merged `cebee80`.
- [x] After merge, cut a dedicated release PR per `RELEASING.md`. **Note:** the version line above (`v0.27.0`) was stale by the time this cut; `main` had already advanced to `0.30.0` by the time #153 merged, and a concurrent, unrelated release PR (#155) claimed `0.30.1` while this release was being cut, so E1 shipped as **`v0.30.2`** via [#156](https://github.com/savvagent/otto/pull/156) instead of the `0.30.0` guessed above.
