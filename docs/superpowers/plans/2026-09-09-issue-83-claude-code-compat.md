...
- [x] Add the tool-name divergence caveat to `README.md` — Claude Code assets name Claude Code tools; prose is left alone, frontmatter tool names that match nothing are dropped with a load-time warning, and a `[compat] tool_aliases` map is a deferred follow-up.
