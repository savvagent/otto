...
- `crates/otto/src/bin/otto-tool-lsp.rs` — 7-line shim that calls `tool_lsp::run()`.
...
- `crates/otto/Cargo.toml` — add `[[bin]] name = "otto-tool-lsp"`; add `tool-lsp.workspace = true` to deps.
- `crates/otto/src/main.rs` — register `tool-lsp` in the default `HostConfig` via a `tool_lsp_path()` resolver and `with_tool(ToolEndpoint::Stdio { … })`.
...
## Task 2: Add `otto-tool-lsp` binary shim

**Files:**
- Create: `crates/otto/src/bin/otto-tool-lsp.rs`
- Modify: `crates/otto/Cargo.toml`

- [ ] **Step 1: Add the bin entry**

In `crates/otto/Cargo.toml`, locate the existing `[[bin]] name = "otto-tool-fs"` block and add a sibling block directly below it:
...
Still in `crates/otto/Cargo.toml`, find the `tool-fs = { workspace = true }` dependency and add directly below it:
...
Run: `rustup run stable cargo build -p otto --bin otto-tool-lsp 2>&1 | tail -10`
Expected: clean build.

- [ ] **Step 5: Smoke-test stdin closure**

Run: `echo '' | rustup run stable cargo run -p otto --bin otto-tool-lsp 2>&1 | tail -10`
...
```bash
git add crates/otto/Cargo.toml crates/otto/src/bin/otto-tool-lsp.rs Cargo.lock
git commit -m "tool-lsp: add otto-tool-lsp binary shim

7-line shim under crates/otto/src/bin/ matching the
otto-tool-fs / -bash / -grep pattern. The release archive
...
2. Add the dispatch shim that:
   - Validates the file is inside `OTTO_TOOL_LSP_ROOT`.
...
Resolves path inside OTTO_TOOL_LSP_ROOT, matches extension to a
...
Then implement the `ServerHandler::initialize` method to stash the peer (or override `on_resource_updated` — verify the exact rmcp 1.6 hook by reading `crates/otto-host/src/tools.rs`'s newly-added `ResourceCapturingHandler` for inspiration; whatever `RequestContext::peer` field is exposed, capture it once via `OnceCell::set`).
...
Run: `grep -n "tool_fs_path\|ToolEndpoint::Stdio\|OTTO_TOOL_FS_BIN" crates/otto/src/main.rs 2>&1 | head -20`
...
In `crates/otto/src/main.rs`, directly below the existing `fn tool_fs_path()` (or whatever the helper is named), add:

```rust
/// Resolve the `otto-tool-lsp` binary the same way we resolve
/// `otto-tool-fs`: `$OTTO_TOOL_LSP_BIN` if set, then
/// `$PATH`-resolve `otto-tool-lsp`, then fall back to the
/// workspace `target/` build artifact.
fn tool_lsp_path() -> std::path::PathBuf {
    if let Ok(p) = std::env::var("OTTO_TOOL_LSP_BIN") {
        return std::path::PathBuf::from(p);
    }
    if let Ok(p) = which::which("otto-tool-lsp") {
        return p;
    }
    // Workspace fallback: the dev target dir.
    let exe = std::env::current_exe().unwrap_or_default();
    if let Some(parent) = exe.parent() {
        let candidate = parent.join("otto-tool-lsp");
        if candidate.exists() {
            return candidate;
        }
    }
    std::path::PathBuf::from("otto-tool-lsp")
}
```
...
```rust
        .with_tool(otto_host::ToolEndpoint::Stdio {
...
Run: `rustup run stable cargo run -p otto -- --print-tools 2>&1 | tail -20`

If `--print-tools` doesn't exist (it's a hypothetical debug flag), use whatever the existing TUI gives for listing tools (`grep -n "fn list_tools\|print_tools\|tools_debug" crates/otto/src/main.rs`). If no such helper, skip this step and rely on the integration tests in Task 17.
...
otto main now adds tool-lsp as a stdio endpoint alongside
tool-fs/grep/bash. Binary is resolved via OTTO_TOOL_LSP_BIN,
...
- **`tool-lsp` crate**. New stdio MCP server that wraps user-configured LSP servers behind seven MCP tools (`lsp_definition`, `lsp_references`, `lsp_hover`, `lsp_document_symbols`, `lsp_workspace_symbols`, `lsp_rename`, `lsp_code_actions`) and publishes diagnostics as MCP resources (`lsp://diagnostics/<absolute-path>`). Configured via `~/.otto/lsp.toml` + `<repo>/.otto/lsp.toml`; no languages are hardcoded.
- **`otto-tool-lsp` binary**. Shim that delegates to `tool_lsp::run`; bundled alongside `otto-tool-fs`/`-bash`/`-grep` in the release archive.
...
- [ ] `otto-tool-lsp` shim binary added under `crates/otto/src/bin/`.
...
- [ ] tool-lsp registered in the default `HostConfig` via `crates/otto/src/main.rs`.
