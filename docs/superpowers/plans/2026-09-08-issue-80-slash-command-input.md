...
- [ ] This task touches `crates/otto/src/main.rs` but not `crates/otto/src/app.rs` or `crates/otto/src/tui.rs`; verify explicitly that the change stays in synchronous key-routing logic and does not hold any host-swap `RwLock` guard across an `.await`.
...
- [ ] Run `cargo test -p otto plugin::effects -- --nocapture`, `cargo test -p otto command_palette::screen -- --nocapture`, `cargo test -p otto palette_shortcut_requires_empty_prompt -- --nocapture`, and `cargo test -p otto egui_app::view::tests -- --nocapture` and expect the targeted suites to pass.
