...
(`crates/otto/src/plugin/builtin/view_file/`, `.../edit_file/`) that push marker screens with
...
plugin ABI (`otto-plugin`) exposes `ScreenArgs::ViewFile`/`EditFile` and `Effect::SaveActiveFile`
as the typed surface backing this; `otto-plugin-wasm`'s `interactive.rs` adapter projects
...
**Tech Stack:** Rust 2024, `otto-plugin`'s `Screen`/`Effect`/`ScreenArgs` ABI types,
`ratatui-code-editor` (removed) / `egui_code_editor` (removed if orphaned), `rust_i18n::t!` + the
four TOML locale files under `crates/otto/locales/`.
...
**Branch:** `otto/remove-view-edit`

**File Map:**

- Modified: `crates/otto-plugin/src/types.rs` — remove `ScreenArgs::ViewFile`/`EditFile` and
  their `screen_id()` arms + tests.
- Modified: `crates/otto-plugin/src/effect.rs` — remove `Effect::SaveActiveFile` + its `Debug`
  arm + tests.
- Modified: `crates/otto-plugin-wasm/src/adapter/interactive.rs` — remove the
  `ScreenArgs::ViewFile`/`EditFile` JSON-projection arms + test fixtures.
- Deleted: `crates/otto/src/plugin/builtin/view_file/` (whole dir),
  `crates/otto/src/plugin/builtin/edit_file/` (whole dir),
  `crates/otto/src/plugin/builtin/editor_keybindings/` (whole dir).
- Modified: `crates/otto/src/plugin/builtin/mod.rs` — remove the three `pub mod` declarations
  and their doc comments (e.g. "Basic in-TUI file editor; opened via `/edit <path>`.").
- Modified: `crates/otto/src/plugin/mod.rs` — remove the three plugin constructions from
  `register_builtins`; update `register_builtins_pr8_complete` and any other test asserting the
  builtin ID list/count.
- Modified: `crates/otto/src/plugin/builtin/themes/editor_theme.rs` — remove
...
- Modified: `crates/otto/src/app.rs` — remove `/view`/`/edit` `Command` entries,
...
- Modified: `crates/otto/src/main.rs` — remove the `top_id == "view-file" || "edit-file"`
  screen-stack special case and the `InputMode::ViewingFile`/`EditingFile` match arms.
- Modified: `crates/otto/src/ui.rs` — remove `paint_file_screen`, its call site, and the
  `InputMode::ViewingFile | InputMode::EditingFile` render blocks.
- Deleted: `crates/otto/src/egui_app/widgets/editor.rs`,
  `crates/otto/src/egui_app/widgets/editor_theme.rs`.
- Modified: `crates/otto/src/egui_app/widgets/mod.rs` — remove `pub mod editor;`/
...
- Modified: `crates/otto/src/egui_app/mod.rs` — remove `editor_buffer` field,
  `save_editor_buffer`, `ensure_buffer_for_active_screen` call, and the `id == "edit-file"` Ctrl-S
  special case.
- Modified: `crates/otto/src/egui_app/view.rs` — remove the
  `id == "view-file" || id == "edit-file"` render branch.
- Modified: `crates/otto/Cargo.toml` — remove `ratatui-code-editor.workspace = true` (and
...
- Modified: `crates/otto/locales/{en,es,hi,pt}.toml` — remove orphaned keys (exact names
...
## Task 1: Remove the plugin-ABI surface (`otto-plugin`, `otto-plugin-wasm`)
...
- [x] Baseline: `cargo test --workspace --no-fail-fast` — confirm green before touching anything, and
      ...
      just the two plugin crates, since Task 2 deletes tests in `otto` too).
- [x] In `crates/otto-plugin/src/types.rs`: delete the `ScreenArgs::ViewFile { path: String }`
...
- [x] In `crates/otto-plugin/src/effect.rs`: delete the `Effect::SaveActiveFile` variant, its
      `Debug` arm (`Effect::SaveActiveFile => f.write_str("SaveActiveFile")`), and any test
      referencing it.
- [x] In `crates/otto-plugin-wasm/src/adapter/interactive.rs`: delete the
      ...
      assertions live).
- [x] Run `cargo check -p otto --all-targets` — expect compile errors pointing at every
      downstream caller (this is expected; Task 2/3/4 fix them). `cargo build`/`cargo check` scoped
      to `-p otto-plugin -p otto-plugin-wasm` would only build those two crates and their
      dependencies, not the reverse-dependent `otto` crate, so it cannot surface these errors —
      use `-p otto --all-targets` instead to pull in the whole downstream graph (including
      ...
      `editor_keybindings` plugin modules, `crates/otto/src/plugin/effects.rs`,
      `crates/otto/src/app.rs`).
- [x] Run `cargo test -p otto-plugin -p otto-plugin-wasm` — expect green (these two crates
...
- [x] Public-interface check: this is the **breaking plugin-ABI change** — record in the PR body
      ...
      `otto-plugin`, per Non-Negotiable Rule 6. Flag explicitly to the architecture reviewer in
      Phase 4 step 8.
...
- [x] Format and commit: `cargo fmt --all` then
      `git commit -m "otto-plugin: remove ScreenArgs::ViewFile/EditFile and Effect::SaveActiveFile"`.

## Task 2: Remove the built-in `view_file`/`edit_file`/`editor_keybindings` plugins and their registration

**Files:**
- Delete: `crates/otto/src/plugin/builtin/view_file/` (dir)
- Delete: `crates/otto/src/plugin/builtin/edit_file/` (dir)
- Delete: `crates/otto/src/plugin/builtin/editor_keybindings/` (dir)
- Modify: `crates/otto/src/plugin/builtin/mod.rs`
- Modify: `crates/otto/src/plugin/mod.rs`

- [x] `rm -rf crates/otto/src/plugin/builtin/view_file crates/otto/src/plugin/builtin/edit_file crates/otto/src/plugin/builtin/editor_keybindings`.
- [x] In `crates/otto/src/plugin/builtin/mod.rs`: remove the `pub mod edit_file;`,
      `pub mod editor_keybindings;`, `pub mod view_file;` declarations.
- [x] In `crates/otto/src/plugin/mod.rs::register_builtins`: remove the three
      ...
      builtin plugin count or enumerating builtin IDs, e.g. via `grep -n "internal:edit-file\|internal:view-file\|internal:editor-keybindings" crates/otto/src/plugin/mod.rs`)
      to drop the three removed IDs and decrement the expected count by 3.
- [x] `cargo check -p otto --all-targets` (this crate won't fully build yet — `app.rs`/`main.rs`/`ui.rs`/
...
- [x] Format and commit (even though the crate doesn't build yet, this repo's task-by-task discipline
      ...
      `git commit -m "otto: remove view-file/edit-file/editor-keybindings builtin plugins"`.
...
- [x] In `crates/otto/src/plugin/effects.rs`: in `apply_one`'s `Effect::CloseScreen` arm, remove
...
- [x] In `crates/otto/src/app.rs`:
  ...
  - Grep `active_file_path` across the whole crate (`grep -rn "active_file_path" crates/otto/src/`)
...
- [x] In `crates/otto/src/plugin/builtin/themes/editor_theme.rs`: remove `build_editor_theme`,
      ...
      **Keep** `pub(crate) fn xterm_256_rgb` and its doc comment (drop only the sentence naming the
      ...
      by `crates/otto/src/egui_app/convert.rs` (`use crate::plugin::builtin::themes::editor_theme::xterm_256_rgb;`)
...
- [x] Run `cargo check -p otto --all-targets` — expect the remaining errors to be confined to
      `crates/otto/src/egui_app/*.rs` (Task 4). If any error remains in `app.rs`/`main.rs`/
...
- [x] ProgressDispatcher check: not applicable — no streaming provider path touched.
- [x] Format and commit: `cargo fmt --all` then
      `git commit -m "otto: remove view-file/edit-file TUI state, key-routing, and rendering"`.

## Task 4: Remove the GUI (egui) frontend's parallel implementation

**Files:**
- Delete: `crates/otto/src/egui_app/widgets/editor.rs`
- Delete: `crates/otto/src/egui_app/widgets/editor_theme.rs`
- Modify: `crates/otto/src/egui_app/widgets/mod.rs`
- Modify: `crates/otto/src/egui_app/mod.rs`
- Modify: `crates/otto/src/egui_app/view.rs`

- [x] Before deleting, confirm `crates/otto/src/egui_app/widgets/editor_theme.rs` is genuinely
      the GUI-only file and not confused with the TUI's similarly-named
      `crates/otto/src/plugin/builtin/themes/editor_theme.rs` (a different file, untouched by
      this plan) — verify by path, not by content similarity.
  - `rm crates/otto/src/egui_app/widgets/editor.rs crates/otto/src/egui_app/widgets/editor_theme.rs`.
- [x] In `crates/otto/src/egui_app/widgets/mod.rs`: remove `pub mod editor;` and
      `pub mod editor_theme;` (keep `pub mod canvas;` and `pub mod file_picker;`).
- [x] In `crates/otto/src/egui_app/mod.rs`: remove the `editor_buffer: Option<widgets::editor::EditorBuffer>`
...
- [x] In `crates/otto/src/egui_app/view.rs`: remove the
...
- [x] Run `cargo build --workspace --all-targets` — expect a fully green build across the entire
      ...
      `crates/otto/src/` to find it before proceeding.
...
- [x] Format and commit: `cargo fmt --all` then
      `git commit -m "otto: remove view-file/edit-file GUI (egui) frontend"`.
...
- Modify: `crates/otto/locales/en.toml`, `es.toml`, `hi.toml`, `pt.toml`
- Modify: `README.md`
- Modify: `PRD.md`
- Modify: `CHANGELOG.md`
...
- [x] Confirm zero remaining references: `grep -rn "ratatui_code_editor\|ratatui-code-editor" crates/ Cargo.toml`
      ...
      `ratatui-code-editor.workspace = true` from `crates/otto/Cargo.toml` and the
...
- [x] Check `egui_code_editor`: `grep -rn "egui_code_editor" crates/` — if the only remaining
      ...
      remove `egui_code_editor.workspace = true` from `crates/otto/Cargo.toml` and the
...
- [x] For each of `crates/otto/locales/{en,es,hi,pt}.toml`: grep for `view-file`, `edit-file`,
...
- [x] Run `cargo test -p otto --test locales` (the locale-parity integration test lives at
      `crates/otto/tests/locales.rs`) to confirm removing keys from all four files in lockstep
...
- [x] In `CHANGELOG.md`: add, under `## [Unreleased]`, a `### Removed` section (create it if it
      ...
      and `Effect::SaveActiveFile` from the `otto-plugin` ABI, with the one-line migration note
...
- [x] Manual smoke check (out-of-band, TUI runtime wiring is touched by this plan): `cargo build --workspace`
      then `cargo run -p otto`, open the command palette (`/`) and confirm `/view`, `/edit`, and
      `/editor-keybindings` no longer appear, `@` still opens the file-reference picker, and
      `/prompt-keybindings` still opens correctly.
- [x] Confirm no `OTTO_TOOL_*` / provider / plugin-ABI-adjacent path was inadvertently touched
...
- [x] **Architecture-reviewer callout (mandatory):** the PR description and the architecture-review
      dispatch (Phase 4 step 8 of the `otto-development` skill) must explicitly name the
      removal of `ScreenArgs::ViewFile`, `ScreenArgs::EditFile`, and `Effect::SaveActiveFile` from
      the `otto-plugin` public ABI, and the removal of the `/view`, `/edit`, and
