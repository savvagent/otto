...
- [ ] Baseline: run `cargo build --workspace --all-targets` on the current tree and confirm it is
      ...
      task).
- [ ] `git mv crates/otto crates/otto`
- [ ] `git mv crates/otto-protocol crates/otto-protocol`
- [ ] `git mv crates/otto-mcp crates/otto-mcp`
- [ ] `git mv crates/otto-host crates/otto-host`
- [ ] `git mv crates/otto-plugin crates/otto-plugin`
- [ ] `git mv crates/otto-plugin-wit crates/otto-plugin-wit`
- [ ] `git mv crates/otto-plugin-wasm crates/otto-plugin-wasm`
- [ ] `git mv crates/otto-fence crates/otto-fence`
- [ ] `git mv crates/otto-canvas crates/otto-canvas`
- [ ] In each moved crate's `Cargo.toml`, change `name = "otto*"` to `name = "otto*"`.
...
- [ ] Sweep every `.rs` file for `use otto_` / `otto_protocol::` / `otto_mcp::` /
...
- [ ] `git add -A && git commit -m "otto: rename otto-* crates to otto-*"`

## Task 2: Rename binaries

**Files:** `crates/otto/Cargo.toml` (`[[bin]]` targets), `crates/otto/src/bin/otto-*.rs` files.

- [ ] `git mv crates/otto/src/bin/otto-anthropic.rs crates/otto/src/bin/otto-anthropic.rs`
      (repeat for gemini, openai, tool-bash, tool-fs, tool-grep, tool-lsp, tool-web).
- [ ] In `crates/otto/Cargo.toml`, rename the `[[bin]] name = "otto"` entry (the main TUI
...
- [ ] `git add -A && git commit -m "otto: rename otto-* binaries to otto-*"`
...
- [ ] Grep for every `OTTO_` env var reference (`OTTO_HOME`, `OTTO_MODEL`,
...
- [ ] Grep for `.otto` (config dir path construction, e.g. `dirs::home_dir().join(".otto")`)
...
- [ ] Grep for `OTTO.md` (project-context filename) and replace with `OTTO.md` in
      `otto-host`'s project-context loader and every doc/comment referencing it.
- [ ] Grep for the keyring service string `"otto"` (distinct from crate/binary names — this is
...
- [ ] Rename the plugin manifest's public `[plugin].otto` key to `[plugin].otto` in
...
- [ ] Rename the public `SourceScope::{ProjectOtto,UserOtto}` enum variants in
...
- [ ] Update installer URL references (`.../savvagent/otto/releases/latest/download/
...
- [ ] `git mv .github/skills/otto-development .github/skills/otto-development` and update the
...
- [ ] Sweep all 7 `.github/workflows/*.yml` files for `otto` occurrences (binary names, crate
      names, cache keys, artifact names, WIT package references not already handled in Task 3) and
      replace with `otto` equivalents.
- [ ] Sweep `.claude/skills/tui-engineer/SKILL.md` for `otto` occurrences and replace.
...
- [ ] Sweep `.gitignore` for `otto`-derived path references (crate directory paths, generated
      file comments) and update them to the new `otto-*` crate directory names.
- [ ] Sweep `crates/otto/locales/{en,es,hi,pt}.toml` for every string mentioning "otto",
...
- [ ] Update `CHANGELOG.md`'s `[Unreleased]` section (only — leave every already-dated, released
      ...
      Task 3's entry.
- [ ] Final verification sweep: `grep -ril "otto" --include="*" . | grep -v -E
...
- [ ] `git add -A && git commit -m "docs: rename otto references to otto across docs, CI, locales, skills"`
