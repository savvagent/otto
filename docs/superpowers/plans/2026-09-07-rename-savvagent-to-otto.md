...
- [x] Baseline: run `cargo build --workspace --all-targets` on the current tree and confirm it is
      ...
      task).
- [x] `git mv crates/savvagent crates/otto`
- [x] `git mv crates/savvagent-protocol crates/otto-protocol`
- [x] `git mv crates/savvagent-mcp crates/otto-mcp`
- [x] `git mv crates/savvagent-host crates/otto-host`
- [x] `git mv crates/savvagent-plugin crates/otto-plugin`
- [x] `git mv crates/savvagent-plugin-wit crates/otto-plugin-wit`
- [x] `git mv crates/savvagent-plugin-wasm crates/otto-plugin-wasm`
- [x] `git mv crates/savvagent-fence crates/otto-fence`
- [x] `git mv crates/savvagent-canvas crates/otto-canvas`
- [x] In each moved crate's `Cargo.toml`, change `name = "savvagent*"` to `name = "otto*"`.
...
- [x] Sweep every `.rs` file for `use savvagent_` / `savvagent_protocol::` / `savvagent_mcp::` /
...
- [x] `git add -A && git commit -m "otto: rename savvagent-* crates to otto-*"`

## Task 2: Rename binaries

**Files:** `crates/otto/Cargo.toml` (`[[bin]]` targets), `crates/otto/src/bin/savvagent-*.rs` files.

- [x] `git mv crates/otto/src/bin/savvagent-anthropic.rs crates/otto/src/bin/otto-anthropic.rs`
      (repeat for gemini, openai, tool-bash, tool-fs, tool-grep, tool-lsp, tool-web).
- [x] In `crates/otto/Cargo.toml`, rename the `[[bin]] name = "savvagent"` entry (the main TUI
...
- [x] `git add -A && git commit -m "otto: rename savvagent-* binaries to otto-*"`
...
- [x] Grep for every `SAVVAGENT_` env var reference (`SAVVAGENT_HOME`, `SAVVAGENT_MODEL`,
...
- [x] Grep for `.savvagent` (config dir path construction, e.g. `dirs::home_dir().join(".savvagent")`)
...
- [x] Grep for `SAVVAGENT.md` (project-context filename) and replace with `OTTO.md` in
      `otto-host`'s project-context loader and every doc/comment referencing it.
- [x] Grep for the keyring service string `"savvagent"` (distinct from crate/binary names — this is
...
- [x] Rename the plugin manifest's public `[plugin].savvagent` key to `[plugin].otto` in
...
- [x] Rename the public `SourceScope::{ProjectSavvagent,UserSavvagent}` enum variants in
...
- [x] Update installer URL references (`.../savvagent/savvagent-cli/releases/latest/download/
...
- [x] `git mv .github/skills/savvagent-development .github/skills/otto-development` and update the
...
- [x] Sweep all 7 `.github/workflows/*.yml` files for `savvagent` occurrences (binary names, crate
      names, cache keys, artifact names, WIT package references not already handled in Task 3) and
      replace with `otto` equivalents.
- [x] Sweep `.claude/skills/tui-engineer/SKILL.md` for `savvagent` occurrences and replace.
...
- [x] Sweep `.gitignore` for `savvagent`-derived path references (crate directory paths, generated
      file comments) and update them to the new `otto-*` crate directory names.
- [x] Sweep `crates/otto/locales/{en,es,hi,pt}.toml` for every string mentioning "savvagent",
...
- [x] Update `CHANGELOG.md`'s `[Unreleased]` section (only — leave every already-dated, released
      ...
      Task 3's entry.
- [x] Final verification sweep: `grep -ril "savvagent" --include="*" . | grep -v -E
...
- [x] `git add -A && git commit -m "docs: rename savvagent references to otto across docs, CI, locales, skills"`
