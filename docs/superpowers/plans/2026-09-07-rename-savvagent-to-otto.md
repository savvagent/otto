# rename-otto-to-otto Implementation Plan

**Goal:** Rename every `otto`-derived crate, binary, on-disk path, keyring service, env var,
...
tool-dispatch behavior changes. `crates/otto` becomes `crates/otto` (the TUI binary + its
`[[bin]]` companions); `crates/otto-{protocol,mcp,host,plugin,plugin-wit,plugin-wasm,fence,canvas}`
become `crates/otto-*`; `provider-*`/`tool-*` crate directories are untouched but their
`otto-*` path/version dependency references change. User-facing surfaces (`~/.otto/` →
`~/.otto/`, `OTTO.md` → `OTTO.md`, keyring service, `OTTO_*` env vars, the WIT package
`otto:plugin@0.1.0` → `otto:plugin@0.1.0`) and docs/CI/locales follow in later tasks once the
...
**Spec:** `docs/superpowers/specs/2026-09-07-rename-otto-to-otto-design.md` — read it first.
...
**Branch:** `rename/otto-to-otto`

## File Map

**Renamed directories (git mv, history preserved)**
- `crates/otto` → `crates/otto`
- `crates/otto-protocol` → `crates/otto-protocol`
- `crates/otto-mcp` → `crates/otto-mcp`
- `crates/otto-host` → `crates/otto-host`
- `crates/otto-plugin` → `crates/otto-plugin`
- `crates/otto-plugin-wit` → `crates/otto-plugin-wit`
- `crates/otto-plugin-wasm` → `crates/otto-plugin-wasm`
- `crates/otto-fence` → `crates/otto-fence`
- `crates/otto-canvas` → `crates/otto-canvas`
- `crates/otto/src/bin/otto-{anthropic,gemini,openai,tool-bash,tool-fs,tool-grep,tool-lsp,tool-web}.rs`
  → `crates/otto/src/bin/otto-{anthropic,gemini,openai,tool-bash,tool-fs,tool-grep,tool-lsp,tool-web}.rs`
- `.github/skills/otto-development/` → `.github/skills/otto-development/`
...
- Every moved crate's own `Cargo.toml` — `name`, and any `otto-*` path dependency.
- Every `use otto_*` / `otto_*::` Rust import across `crates/`, `examples/`.
- `crates/otto-host/src/{config,default_prompt,permissions,project,session}.rs`,
  `crates/otto-plugin-wasm/src/discovery.rs` — `~/.otto/`, `OTTO.md`, keyring service,
  `OTTO_*` env vars.
...
`otto_*` `use`/path reference.

- [x] Baseline: run `cargo build --workspace --all-targets` on the current tree and confirm it is
      green before making any change (guards against attributing a pre-existing failure to this
      task).
- [x] `git mv crates/otto crates/otto`
- [x] `git mv crates/otto-protocol crates/otto-protocol`
- [x] `git mv crates/otto-mcp crates/otto-mcp`
- [x] `git mv crates/otto-host crates/otto-host`
- [x] `git mv crates/otto-plugin crates/otto-plugin`
- [x] `git mv crates/otto-plugin-wit crates/otto-plugin-wit`
- [x] `git mv crates/otto-plugin-wasm crates/otto-plugin-wasm`
- [x] `git mv crates/otto-fence crates/otto-fence`
- [x] `git mv crates/otto-canvas crates/otto-canvas`
- [x] In each moved crate's `Cargo.toml`, change `name = "otto*"` to `name = "otto*"`.
- [x] In the root `Cargo.toml`, update `[workspace] members` and `default-members` to the new
      ...
      (`otto-plugin`, `otto-plugin-wit`, `otto-plugin-wasm`, `otto-protocol`,
      `otto-mcp`, `otto-host`, `otto-fence`, `otto-canvas`) to its `otto-*` name
      and `path`.
  - Note: `crates/otto-host/tests/fixtures/resource-tool` and
...
- [x] Sweep every `.rs` file for `use otto_` / `otto_protocol::` / `otto_mcp::` /
      `otto_host::` / `otto_plugin` (and `_wit`/`_wasm` variants) /
      `otto_fence::` / `otto_canvas::` and replace the crate-name segment with `otto_*`
      (Rust crate names use underscores, so `otto-host` → `otto_host` in `use` paths). Use a
      scripted `grep -rl` + `sed -i 's/otto_/otto_/g'` pass across `crates/` and `examples/`,
      then hand-verify with `cargo check --workspace` — sed cannot distinguish "this identifier
      happens to contain otto_" from "this is a crate path", so re-check compile errors
      surface any place the substitution over- or under-fired.
- [x] `cargo check --workspace` until clean; this regenerates `Cargo.lock` — do not hand-edit it.
- [x] `cargo build --workspace --all-targets` — must succeed.
- [x] Public-interface note: this task alone is not yet user-visible (crate names are an internal
...
- [x] `git add -A && git commit -m "otto: rename otto-* crates to otto-*"`

## Task 2: Rename binaries

**Files:** `crates/otto/Cargo.toml` (`[[bin]]` targets), `crates/otto/src/bin/otto-*.rs` files.

- [x] `git mv crates/otto/src/bin/otto-anthropic.rs crates/otto/src/bin/otto-anthropic.rs`
      (repeat for gemini, openai, tool-bash, tool-fs, tool-grep, tool-lsp, tool-web).
- [x] In `crates/otto/Cargo.toml`, rename the `[[bin]] name = "otto"` entry (the main TUI
...
- [x] Grep the tree for any remaining reference to the old binary names as strings (not just crate
      paths) — e.g. `HostConfig::with_tool` wiring in `crates/otto/src/main.rs` that resolves
      `otto-tool-fs` via `$PATH`, and any test that spawns a binary by its old name.
- [x] `cargo build --workspace --all-targets` — confirm every renamed `[[bin]]` target builds and
      the binary artifacts appear under `target/debug/` with the new names
      (`ls target/debug/otto target/debug/otto-tool-fs` etc.).
- [x] Public-interface note: binary names are user-facing (installer scripts, `$PATH` entries,
      `OTTO_TOOL_*_BIN` overrides depend on them) — flag as part of the Task 3 breaking-change
      CHANGELOG entry, not duplicated here.
- [x] `git add -A && git commit -m "otto: rename otto-* binaries to otto-*"`
...
- [x] Grep for every `OTTO_` env var reference (`OTTO_HOME`, `OTTO_MODEL`,
      `OTTO_PROJECT_DIR`, `OTTO_PROVIDER_URL`, `OTTO_KEYRING_SERVICE`,
      `OTTO_AGENT_MAX_DEPTH`, `OTTO_NO_UPDATE_CHECK`,
      `OTTO_ALLOW_MISSING_LIVE_KEYS`, `OTTO_TOOL_*_BIN`/`_ROOT`,
      `OTTO_{ANTHROPIC,GEMINI,OPENAI}_LISTEN`, `OTTO_BRAVE_API_KEY`,
      `OTTO_SEARXNG_URL`, `OTTO_TURN_ID__`) across `crates/`, `docs/`, `README.md`,
      `.env`/`.env.local` examples and replace `OTTO_` → `OTTO_`.
- [x] Grep for `.otto` (config dir path construction, e.g. `dirs::home_dir().join(".otto")`)
...
- [x] Grep for `OTTO.md` (project-context filename) and replace with `OTTO.md` in
      `otto-host`'s project-context loader and every doc/comment referencing it.
- [x] Grep for the keyring service string `"otto"` (distinct from crate/binary names — this is
...
- [x] Rename the WIT package in all five `crates/otto-plugin-wit/wit/*.wit` files:
      `package otto:plugin@0.1.0;` → `package otto:plugin@0.1.0;`, and update every `use
      otto:plugin/...` import within those same files.
- [x] Update `examples/plugin-hello-{interactive,provider,static}` (or wherever their WIT bindings
      reference the old package) to the new `otto:plugin@0.1.0` package.
- [x] Update `.github/workflows/wit-dep-guard.yml` and `.github/workflows/wit-portability-guard.yml`
      for the new WIT package name wherever they reference it literally.
- [x] Rename the plugin manifest's public `[plugin].otto` key to `[plugin].otto` in
      `crates/otto-plugin-wasm/src/manifest.rs` (`ManifestPlugin::otto` field → `::otto`,
      ...
      `otto = "^0.18"` (etc.) to `otto = "^0.18"`.
- [x] Rename the public `SourceScope::{ProjectOtto,UserOtto}` enum variants in
...
- [x] Rebuild the committed `.wasm` test fixtures under
...
- [x] Rename the hardcoded GitHub repository identifiers used by the self-update and in-app
      ...
      URL constant and its test assertion — all from `savvagent/otto` to `savvagent/otto`.
- [x] Verify the provider-transport-split env var rename explicitly (not just via compilation):
      confirm existing tests covering `OTTO_PROVIDER_URL` (now `OTTO_PROVIDER_URL`) still pass
...
- [x] Update installer URL references (`.../savvagent/otto/releases/latest/download/
      otto-installer.sh` and `.ps1`) to `.../savvagent/otto/releases/latest/download/
      otto-installer.sh` in `README.md` and any install script under the repo.
- [x] Rebuild and run the plugin loader test path: `cargo build --workspace --all-targets`, then
...
- [x] `cargo test --workspace` — full pass, confirming project-context loading
...
- [x] Public-interface change (breaking, comprehensive): add a `CHANGELOG.md` `[Unreleased]` entry
...
- [x] `git add -A && git commit -m "otto: rename user-facing surfaces (env vars, paths, keyring, WIT package) to otto"`
...
`.github/workflows/*.yml` (all 7), `.github/skills/otto-development/` →
`.github/skills/otto-development/`, `.claude/skills/tui-engineer/SKILL.md`,
`crates/otto/locales/{en,es,hi,pt}.toml`.

- [x] `git mv .github/skills/otto-development .github/skills/otto-development` and update the
      skill's own internal self-references (repo name, `savvagent/otto` → `savvagent/otto`,
      crate names, branch/commit-scope examples) throughout `SKILL.md` and `agent-prompts.md`.
- [x] Sweep `README.md`, `PRD.md`, `CLAUDE.md`, `RELEASING.md`, `SECURITY.md`,
      `crates/otto-protocol/SPEC.md`, `Justfile`, `bacon.toml`, `release-plz.toml` for every
      remaining `otto`/`OTTO` occurrence (repo name, crate names, binary names, paths, env
      vars, keyring service) and replace with the `otto` equivalent. Preserve the GitHub org handle
      `otto` unchanged (only the repo-name portion of URLs changes:
      `savvagent/otto` → `savvagent/otto`).
- [x] Sweep `docs/**` (including `docs/plugins/authoring.md`, `docs/superpowers/specs/*`,
      `docs/superpowers/plans/*`, excluding this plan's and this task's own file, which are already
      correct) for `otto`/`OTTO` occurrences and rewrite in place, per issue decision 5 —
...
- [x] Sweep all 7 `.github/workflows/*.yml` files for `otto` occurrences (binary names, crate
      names, cache keys, artifact names, WIT package references not already handled in Task 3) and
      replace with `otto` equivalents.
- [x] Sweep `.claude/skills/tui-engineer/SKILL.md` for `otto` occurrences and replace.
- [x] Sweep every other tracked file under `.claude/**` (e.g. `.claude/skills/rust-engineer/SKILL.md`)
      for `otto` occurrences and replace, not just `tui-engineer/SKILL.md`.
- [x] Sweep `.gitignore` for `otto`-derived path references (crate directory paths, generated
      file comments) and update them to the new `otto-*` crate directory names.
- [x] Sweep `crates/otto/locales/{en,es,hi,pt}.toml` for every string mentioning "otto",
      `~/.otto/...`, `otto-tool-*`, or `OTTO_TOOL_*_BIN` and update all four locale
...
- [x] Update `CHANGELOG.md`'s `[Unreleased]` section (only — leave every already-dated, released
      entry above it referring to `otto` as the historical record of what shipped under that
      name) with any additional docs/CI/locale-specific notes, if not already fully covered by
      Task 3's entry.
- [x] Final verification sweep: `grep -ril "otto" --include="*" . | grep -v -E
      ...
      `otto` GitHub org handle, spot-checked rather than blanket-flagged, and (b) dated,
...
- [x] `cargo build --workspace --all-targets` — final green build.
- [x] `cargo test --workspace --no-fail-fast` — final green test run.
- [x] `cargo clippy --workspace --all-targets` — no new warnings (`RUSTFLAGS=-D warnings` in CI).
- [x] `cargo fmt --all --check` — clean.
- [x] `cargo test -p otto-host --test cross_vendor_history --no-fail-fast` — host-plumbing surface
      changed (project-context filename, config paths), so this regression suite runs explicitly.
- [x] Out-of-band verification note (executed at Phase 5, not as a task here, but flagged for that
...
- [x] Update the local `origin` git remote to `https://github.com/savvagent/otto.git` (a
...
- [x] `git add -A && git commit -m "docs: rename otto references to otto across docs, CI, locales, skills"`
...
this plan's PR merges to `main` (otto-development skill Non-Negotiable Rule 8 / Phase 4 step
