...
- [ ] Add `crates/otto-plugin-wasm/wit/` as a byte-for-byte vendored copy of `crates/otto-plugin-wit/wit/`, and add `crates/otto-plugin-wasm/build.rs` that (a) emits `cargo::rerun-if-changed=wit`, (b) watches `../otto-plugin-wit/wit` when the canonical sibling tree exists, (c) fails with actionable messages when the vendored files are missing or differ while the canonical sibling tree exists, and (d) validates the vendored file set plus pinned normalized-content digests when the canonical sibling tree is absent. Expected result: both normal workspace builds and packaged-isolation builds fail early if the vendored WIT inputs are wrong.
