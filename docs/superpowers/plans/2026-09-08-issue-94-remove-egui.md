...
- [ ] Now assert the full completeness check that Task 1 deferred: `rg 'egui|eframe' crates/ -g '*.rs'` must return no hits, and `rg -i 'egui|otto gui' README.md` must return no hits. `docs/` and the **three** historical `CHANGELOG.md` mentions inside already-shipped release sections (`:28` in `## 0.26.4`, `:198` in `## 0.21.0`, and `:235-236`) are expected to still match and must be left alone — shipped release sections are history, not current prose, and are never edited. The `0.26.4` one is new: it shipped while this branch was open, so re-confirm the line numbers with `grep -n -i 'egui\|eframe' CHANGELOG.md` rather than trusting the ones written here.
...
- [ ] Note for the release PR author: `## [Unreleased]` was empty at branch time even though #89 (`provider-deepseek: classify list_models HTTP errors by status code`) and #92 (`otto: mirror highlighted slash command into the prompt input`) had merged after the v0.26.3 tag. That gap has since closed on `main`: both are documented under `## 0.26.4` in `CHANGELOG.md`, which shipped while this branch was open. So `## [Unreleased]` now holds only this change's `### Removed` entry, and v0.27.0 is cut from a `0.26.4` baseline.
