...
- [x] Read `crates/provider-deepseek/{Cargo.toml,src/api.rs,src/translate.rs,src/stream.rs,
      src/mcp.rs,src/lib.rs,tests/integration.rs}` in full — these are the files to mirror.
- [x] Create `crates/provider-grok/Cargo.toml`: same `[package]` shape as
...
- [x] Copy `provider-deepseek/src/api.rs` to `provider-grok/src/api.rs` verbatim (Grok's
...
- [x] Copy `provider-deepseek/src/stream.rs` to `provider-grok/src/stream.rs` verbatim, updating
...
- [x] Copy `provider-deepseek/src/translate.rs` to `provider-grok/src/translate.rs`, renaming
...
- [x] Copy `provider-deepseek/src/mcp.rs` to `provider-grok/src/mcp.rs`, renaming
...
- [x] Write `crates/provider-grok/src/lib.rs` by copying `provider-deepseek/src/lib.rs` and
...
- [x] Adjust the copied `list_models_tests` module (in `lib.rs`, same as `provider-deepseek`'s):
...
- [x] Copy `provider-deepseek/tests/integration.rs` to `provider-grok/tests/integration.rs`,
...
- [x] Add `crates/provider-grok` to root `Cargo.toml`'s `[workspace] members` (alphabetical slot:
...
- [x] Run targeted validation: `cargo test -p provider-grok`. Expect all unit + integration tests
      to pass.
- [x] Run `cargo build -p provider-grok --all-targets` to confirm the new crate compiles cleanly on
      its own before wiring it into `crates/otto`.
- [x] Public-interface check: this task adds a new library crate and new optional env vars
...
- [x] Host-swap `RwLock` check: not applicable — this task touches no `app.rs`/`tui.rs` code.
- [x] ProgressDispatcher check: the `ProgressDispatcher` forwarder-abort pattern lives in
...
- [x] Read `crates/otto/src/plugin/builtin/provider_deepseek/mod.rs` in full — this is the file to
      mirror.
- [x] Write failing tests first in a new `crates/otto/src/plugin/builtin/provider_grok/mod.rs`:
      ...
      below exists.
- [x] Implement the production code in the same file: `PLUGIN_ID = "internal:provider-grok"`,
...
- [x] Register the new submodule in `crates/otto/src/plugin/builtin/mod.rs` (add `pub(crate) mod
...
- [x] Add `provider-grok = { workspace = true }` to `crates/otto/Cargo.toml`'s `[dependencies]`
      (alphabetical slot, after `provider-gemini`, before `provider-local`) — required before the
      plugin module above will compile.
- [x] Add a `[[bin]] name = "otto-grok" path = "src/bin/otto-grok.rs"` target to
      `crates/otto/Cargo.toml`, inserted after the `otto-deepseek` bin target.
- [x] Create `crates/otto/src/bin/otto-grok.rs` as a 10-line shim copied from
      `otto-deepseek.rs`, calling `provider_grok::run()`.
- [x] Run targeted tests: `cargo test -p otto -- provider_grok::`. Expect all five tests to pass.
- [x] Run `cargo build -p otto --bin otto-grok` to confirm the new shim binary compiles and links
      against the new crate.
- [x] Public-interface check: a new built-in provider plugin id (`internal:provider-grok`) and a
      new shipped binary (`otto-grok`) — additive.
- [x] Host-swap `RwLock` check: not applicable — this plugin module holds no `Arc<RwLock<...>>`
...
- [x] ProgressDispatcher check: not applicable at this layer — already verified at the crate level
      in Task 1.
- [x] Format and commit: `cargo fmt --all` then
...
- [x] Write/adjust the failing test first in `crates/otto/src/plugin/effects.rs`: extend the
      `build_connect_candidates` test's `for expected in [...]` array to include `"grok"` alongside
      `"anthropic"`, `"gemini"`, `"openai"`, `"deepseek"`, `"local"`.
- [x] Run the targeted test before implementation: `cargo test -p otto -- build_connect_candidates`
      (or the exact test function name in that module). Expect failure — `"grok"` is not yet a
      connect candidate.
- [x] In `crates/otto/src/plugin/mod.rs`, add
...
- [x] In that same file's `register_builtins_pr8_complete` test, update the hardcoded counts this
...
- [x] In `crates/otto/src/main.rs`'s `bootstrap_pool_host`, add `provider_grok::ProviderGrokPlugin`
...
- [x] In both of `crates/otto/src/main.rs`'s reconnect `match` statements (the ones containing
      ...
      `"deepseek"` arm.
- [x] In `crates/otto/src/providers.rs`, add a `ProviderSpec` entry to the `PROVIDERS` const,
      ...
      ```
- [x] In `crates/otto/src/migration.rs`, add `"grok"` to the `KNOWN_PROVIDERS` const array, after
      `"deepseek"`.
- [x] Run targeted validation after implementation: `cargo test -p otto -- build_connect_candidates`
      ...
      built-ins).
- [x] Grep for any other hardcoded provider-id enumeration that might need the same addition:
...
- [x] Run full workspace tests: `cargo test --workspace`. Expect all tests to pass, including
      `provider-grok`'s own suite from Task 1 and `provider_grok`'s plugin tests from Task 2.
- [x] Public-interface check: additive only — a new provider id surfaces in `/connect`, `/model`,
      `/use`, and `@provider:model` routing; no existing provider id, tool schema, or slash command
      changes shape.
- [x] Host-swap `RwLock` check: `main.rs`'s `bootstrap_pool_host` and reconnect match arms run
      ...
      in that code path.
- [x] Run full required validation: `cargo build --workspace --all-targets`; `cargo test
...
- [x] Update the Install section's provider-count prose (README lines ~14-19 at plan time): "Each
...
- [x] Update the `crates/otto` workspace-map table row (README ~line 56 at plan time): "All ten
...
- [x] Add a `crates/provider-grok` row to the workspace-map table immediately after the
...
- [x] Grep for any provider-id enumeration in prose/examples that lists all built-ins (not
      ...
      list.
- [x] Grep for a "standalone provider" binary-invocation section or env-var table that enumerates
...
- [x] No test command applies to a documentation-only task; visually diff the rendered table
      (`git diff README.md`) to confirm no unrelated row was altered.
- [x] Format and commit: `git commit -m "docs: document the xAI Grok provider in README"` (no
