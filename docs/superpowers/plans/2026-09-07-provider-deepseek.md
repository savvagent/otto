...
- [x] Read `crates/provider-openai/{Cargo.toml,src/api.rs,src/translate.rs,src/stream.rs,
      src/mcp.rs,src/lib.rs,tests/integration.rs}` in full — these are the files to mirror.
- [x] Create `crates/provider-deepseek/Cargo.toml`: same `[package]` shape as
...
- [x] Copy `provider-openai/src/api.rs` to `provider-deepseek/src/api.rs` verbatim (DeepSeek's
      chat-completion request/response schema is identical to OpenAI's) — only update doc
      comments that say "OpenAI" to say "DeepSeek".
- [x] Copy `provider-openai/src/stream.rs` to `provider-deepseek/src/stream.rs` verbatim, updating
...
- [x] Copy `provider-openai/src/translate.rs` to `provider-deepseek/src/translate.rs`, renaming
...
- [x] Copy `provider-openai/src/mcp.rs` to `provider-deepseek/src/mcp.rs`, renaming
...
- [x] Write `crates/provider-deepseek/src/lib.rs` by copying `provider-openai/src/lib.rs` and
...
- [x] Adjust the copied `list_models_tests` module (moved into `lib.rs`, same as
      ...
      `o1-mini`).
- [x] Copy `provider-openai/tests/integration.rs` to `provider-deepseek/tests/integration.rs`,
...
- [x] Add `crates/provider-deepseek` to root `Cargo.toml`'s `[workspace] members` (alphabetical
...
- [x] Run targeted validation: `cargo test -p provider-deepseek`. Expect all unit + integration
      tests to pass.
- [x] Run `cargo build -p provider-deepseek --all-targets` to confirm the new crate compiles
      cleanly on its own before wiring it into `crates/otto`.
- [x] Public-interface check: this task adds a new library crate and a new optional env var
...
- [x] Read `crates/otto/src/plugin/builtin/provider_openai/mod.rs` in full — this is the file to
      mirror.
- [x] Write failing tests first in a new
      ...
      below exists.
- [x] Implement the production code in the same file: `PLUGIN_ID =
...
- [x] Add `provider-deepseek = { workspace = true }` to `crates/otto/Cargo.toml`'s
...
- [x] Add a `[[bin]] name = "otto-deepseek" path = "src/bin/otto-deepseek.rs"` target to
      `crates/otto/Cargo.toml`, inserted after the `otto-openai` bin target.
- [x] Create `crates/otto/src/bin/otto-deepseek.rs` as a 10-line shim copied from
      `otto-openai.rs`, calling `provider_deepseek::run()`.
- [x] Add `otto-deepseek` rows to both `crates/otto/Cargo.toml`'s `[package.metadata.deb]`
      ...
      in each list.
- [x] Run targeted tests: `cargo test -p otto -- provider_deepseek::`. Expect all five tests to
      pass.
- [x] Run `cargo build -p otto --bin otto-deepseek` to confirm the new shim binary compiles and
      links against the new crate.
- [x] Public-interface check: a new built-in provider plugin id (`internal:provider-deepseek`) and
...
- [x] In that same file's `register_builtins_pr8_complete` test, update the hardcoded counts this
      ...
      to `5`.
- [x] In `crates/otto/src/main.rs`'s `bootstrap_pool_host`, add
...
- [x] In both of `crates/otto/src/main.rs`'s reconnect `match` statements (the ones currently
...
- [x] In `crates/otto/src/providers.rs`, add a `ProviderSpec` entry to the `PROVIDERS` const,
      ...
      ```
- [x] In `crates/otto/src/migration.rs`, add `"deepseek"` to the `KNOWN_PROVIDERS` const array,
      after `"openai"`.
...
- [x] Update the provider-count prose from "three standalone provider MCP servers" to "four
      standalone provider MCP servers".
- [x] Update the `crates/otto` workspace-map table row to list
      `otto-{anthropic,gemini,openai,deepseek}` instead of `otto-{anthropic,gemini,openai}`.
- [x] Add a `crates/provider-deepseek` row to the workspace-map table immediately after the
...
- [x] Format and commit: `git commit -m "docs: document the DeepSeek provider in README"` (no
