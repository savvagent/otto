...
- [x] **Step 2: Add a failing test for Gemini (red)**
...
- [x] **Step 3: Extract and fix Gemini's status mapping (green)**
...
- [x] **Step 2: Add the enum and a failing test (red)**
...
- [x] **Step 3: Implement the branching (green)**
...
- [x] **Step 1: Read all four `try_build_registration` implementations**
...
- [x] **Step 3: Update `provider_local::try_build_registration` first (simplest, no credentials/env fallback)**
...
- [x] **Step 4: Update the three cloud shims (Anthropic, Gemini, OpenAI)**
...
- [x] **Step 2: Add `startup_verbose` to `HostBoot` and `App`**
...
- [x] **Step 3: Gate the `try_provider!` macro's notes**
...
- [x] **Step 1: `try_provider!` macro (startup)**
...
- [x] **Step 2: `perform_connect` (explicit `/connect`)**
...
- [x] **Step 3: `apply_pending_pool_add` — add `startup: bool` parameter**
...
- [x] **Step 4: Update all four call sites of `apply_pending_pool_add`**
...
- [x] **Step 5: Build and fix remaining compile errors**
...
- [x] **Step 1: Establish a test seam before writing tests**
...
- [x] **Step 2: Add failing tests (red)**
...
- [x] **Step 1: Add `ProviderLease::display_name()`**
...
- [x] **Step 2: Add a failing test for the new `Display` format (red)**
...
- [x] **Step 3: Change the variant shape and `Display` impl (green)**
...
- [x] **Step 4: Confirm the TUI rendering picks it up unchanged**
...
- [ ] **Step 5: Manual smoke test (documented, not automated)** — not performed; no live API keys available in this environment (see "Implementation notes" below).
...
- [x] Update spec `Status:` header from `pending review` to `IMPLEMENTED` in a final doc-only commit once the PR is open (or once merged, per repo convention — check other recent specs for the exact convention used).
...
- Task 6 (env-var credential recognition) was implemented together with
  Task 3 (`ProviderBuildOutcome` in the four shims) in a single commit,
  since both touch the same `try_build_registration` functions — this is a
  commit-grouping deviation, not a scope deviation. The dedicated
  mock/injectable test seam called for in Task 6 Step 1 was not added;
  env-var fallback behavior is exercised indirectly through the shims'
  existing structure rather than via injected fakes. This is a known
  coverage gap, called out in the PR description.
- `apply_pending_pool_add` has **five** call sites, not four as originally
  scoped: an additional call site in `crates/otto/src/egui_app/mod.rs`'s
  `drain_pending()` (used by the egui GUI frontend) was discovered only via
  compile errors during Task 5 and updated (`startup: false`) alongside the
  four documented sites.
- Step 5 of Task 8 (manual smoke test) was not performed — no live API keys
  are available in this environment. Coverage of the startup/`/connect`
  behavior relies on the unit and integration tests added in Tasks 1-7
  instead; this is noted as a manual-verification gap in the PR description.
