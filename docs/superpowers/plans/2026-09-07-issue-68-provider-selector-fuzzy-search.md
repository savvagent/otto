...
**Architecture:** The provider catalog remains `crate::providers::effective_providers()`; this plan adds small selector/search helpers around that catalog rather than creating a second registry. `crates/otto/src/app.rs` owns the selector query state plus filtered-selection helpers, `crates/otto/src/main.rs` continues to own the keyboard event loop and credential branching, `crates/otto/src/ui.rs` renders the same modal with an optional filter row / empty state, and the coupled `Effect::PromptApiKey` path in `crates/otto/src/plugin/effects.rs` passes the matched provider spec into the unchanged API-key modal. No `otto-host` changes, no transport changes, and no secret-storage changes.
...
- `crates/otto/src/plugin/effects.rs` — keep `Effect::PromptApiKey` aligned with the spec-based API-key helper and runtime provider catalog.
...
- [ ] In `crates/otto/src/plugin/effects.rs`, keep `Effect::PromptApiKey` on the same spec-based API-key entry path as `/connect`, using the runtime `effective_providers()` catalog so plugin-triggered prompts preserve the same placeholder behavior and provider lookup semantics.
