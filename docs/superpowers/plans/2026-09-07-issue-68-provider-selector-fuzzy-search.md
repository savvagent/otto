...
- [ ] Re-run `cargo test -p otto --bin otto provider_selector` and expect the selector helper tests to pass.
...
- [ ] Re-run `cargo test -p otto --bin otto connect_provider_selector` and expect the new selector UI/input tests to pass.
...
- [ ] Host-swap `RwLock` check: this task touches `crates/otto/src/main.rs`; preserve the existing discipline that any `Arc<RwLock<Option<Arc<Host>>>>` guard is cloned/dropped before `.await`, and do not add any new host-lock-taking code to the selector branch.
- [ ] ProgressDispatcher check: not applicable — no streaming provider path is touched.
