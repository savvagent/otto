# Otto — Product Requirements Document
...
**Otto is a blazingly fast, open-source terminal coding agent — written end-to-end in Rust, with every LLM provider and every tool implemented as a Model Context Protocol (MCP) server.**
...
If [OpenCode](https://opencode.ai/) is the reference experience, Otto's pitch is the same UX with three things that matter:
...
2. **MCP-native.** OpenCode treats MCP as one tool source among many; Otto treats MCP as *the* wire format for both tools *and* providers. One transport story to learn.
3. **Provider-as-binary.** Forking Otto to add a model means publishing a new crate, not patching the host.
...
Otto's bet: by collapsing both halves onto MCP and writing the host in Rust, we get a smaller core, a sharper extension story, and a TUI that feels instant.

---

## 3. Inspiration: OpenCode

The diagram below is the OpenCode system architecture, adapted as our reference. Otto keeps most of these layers but flattens the Provider System and Tool System onto the same protocol.
...
- **Project context** loaded from a well-known file (OpenCode uses `AGENTS.md`; Otto will use `OTTO.md`).
...
- A clean separation in which `otto-host` knows nothing about Anthropic, and `provider-anthropic` knows nothing about the TUI.
- A protocol (SPP) frozen enough to publish v0.1 on crates.io.
- Installing Otto on Linux/macOS/Windows should be downloading a single archive (or running a one-line install script) plus authenticating with a provider — either an env var like `ANTHROPIC_API_KEY` or running `/connect` once to store the key in the OS keyring. Each platform archive bundles the TUI plus the `otto-tool-fs` / `otto-anthropic` / `otto-gemini` MCP servers under a single installer.
- **User-defined subagents.** Users drop markdown files into `.otto/agents/` (or `.claude/agents/` for Claude-Code compat) to make them available as subagents the parent model can spawn via the built-in `task` tool, with per-agent tool scoping and optional model override. Subagent execution gets its own session state, system prompt, and tool view while sharing the parent's provider client, registry, hook gate, and permissions.
...
- Becoming a generic MCP IDE shell. Otto is opinionated about being an *agent* host, not a tool browser.
...
flowchart TB
    subgraph Client["Client Layer"]
        TUI["Otto TUI<br/>(crates/otto — ratatui)"]
    end

    subgraph Host["Host Layer — crates/otto-host"]
        Engine["Agent Engine<br/>(tool-use loop, planning, action)"]
        State["Session State<br/>(history, transcripts)"]
        Project["Project Context<br/>(OTTO.md)"]
        PC["ProviderClient<br/>(Box&lt;dyn&gt;)"]
        TR["ToolRegistry<br/>(stdio children)"]
    end

    subgraph Wire["Wire Layer"]
        SPP["otto-protocol (SPP)<br/>shared types"]
        MCP["otto-mcp<br/>traits, ChannelEmitter,<br/>InProcessProviderClient, rmcp glue"]
    end

    subgraph Providers["Providers — in-process by default"]
        PA["provider-anthropic<br/>(also bin: otto-anthropic)"]
        PG["provider-gemini<br/>(also bin: otto-gemini)"]
...
|---|---|---|
| Client | `otto` | TUI rendering, input handling, talks to host in-process via Rust API |
| Host | `otto-host` | Conversation state, tool-use loop, MCP client orchestration, project context |
| Wire | `otto-protocol`, `otto-mcp` | Shared SPP types, `ProviderHandler` / `ProviderClient` traits, `InProcessProviderClient` bridge, rmcp glue |
...
└── crates/
    ├── otto/                ← TUI (formerly src/)
    ├── otto-protocol/       ← SPP wire types + SPEC.md
    ├── otto-mcp/            ← shared traits, ChannelEmitter, InProcessProviderClient
    ├── otto-host/           ← engine, ProviderClient + ToolRegistry, project context
    ├── provider-anthropic/       ← Anthropic provider + otto-anthropic bin
    ├── provider-gemini/          ← Gemini provider + otto-gemini bin
    └── tool-fs/                  ← filesystem tools + otto-tool-fs bin
```

---

## 6. The wire: Otto Provider Protocol (SPP)

The contract between host and provider servers is **SPP v0.1.0**, defined in `crates/otto-protocol/SPEC.md`. Headline points:
...
- Streaming via MCP `notifications/progress` carrying `StreamEvent`s, gated by `STREAM_EVENT_KIND = "otto/stream-event"`.
- Optional `list_models`, `count_tokens` tools.

Hosts must not require optional tools. Providers configure auth out-of-band.

See `crates/otto-protocol/SPEC.md` for the complete spec and JSON schemas.
...
- `otto-protocol` v0.1.0 with round-trip tests.
- `otto-mcp` `ProviderHandler` / `ProviderClient` / `StreamEmitter` traits + `ChannelEmitter` + `InProcessProviderClient` bridge.

### M2 · Anthropic provider (✅ done)
- `provider-anthropic` library implements `ProviderHandler`; the `otto-anthropic` bin wraps it as an `rmcp` Streamable HTTP server.
- Host links the provider in-process by default; the HTTP path is opt-in via `OTTO_PROVIDER_URL` and exists primarily for wire-protocol debugging.
...
- `read_file`, `write_file`, `list_dir`, `glob` ship as a stdio MCP server (`otto-tool-fs`), spawned and reaped by `ToolRegistry`.

### M4 · `otto-host` engine (✅ done)
- `Host` owns conversation state, the tool-use loop (`run_turn_streaming`), in-memory transcripts, and project context (`OTTO.md`).
...
### M5 · `otto` TUI on the host (✅ done)
- TUI routes every turn through `otto-host` with a streaming-token render path; transcripts persist to `~/.otto/transcripts/<unix>.json`.
...
- `tool-fs` confines paths to `OTTO_TOOL_FS_ROOT` (set by the host to the project root); rejects `..`, symlink escapes, and out-of-root absolute paths. Closes the v0.1 §9 "Layer 1 path hygiene" gap.
...
1. **It works.** A new user can download a precompiled `otto` binary, authenticate with a provider (env var or `/connect`), run it inside a project, and hold a multi-turn conversation that reads/writes files. Crates.io publication is *not* required for v0.1.
...
- **Project context format.** *Resolved in M9.* `OTTO.md` gains optional YAML front-matter for permission overrides; the body remains free-form Markdown injected into the system prompt. Front-matter parse errors fall back silently to "no front-matter" so a malformed file never blocks startup.
...
- **SPP** — Otto Provider Protocol. A small layering on top of MCP defining the `complete` tool's request/response/event shapes. See `crates/otto-protocol/SPEC.md`.
...
- **Host** — `otto-host`. Owns conversation state, runs the tool-use loop, multiplexes provider + tool MCP clients.
- **Client** — for v0.1, the `otto` TUI. Future: desktop / IDE.
