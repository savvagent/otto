...
**Adding a new skill under `.github/skills/` requires a `.claude/skills/<name>/SKILL.md` adapter stub in the same change.** `.github/scripts/check-claude-skill-stubs.sh`, run by CI's `lint` job, fails the build if a stub is missing, if a stub's `name`/`description` have drifted from the canonical, if the stub's pointer line does not name every Markdown file in its canonical directory (companions like `agent-prompts.md` included), or if it names a path that does not exist. The pointer must be one unwrapped line — the check reads only that line, so a path reflowed onto a continuation line counts as unnamed. Run it locally before pushing with `bash .github/scripts/check-claude-skill-stubs.sh`.
