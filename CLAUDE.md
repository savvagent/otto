...
- `.claude/skills/<name>/` is what **Claude Code** discovers (it reads project skills from nowhere else), and holds two kinds of entry: Claude-Code-native skills with no canonical counterpart (`rust-engineer`, `tui-engineer`, `otto-scanner`), and **ports** of a canonical body (`otto-development`, `creating-github-issues`).
