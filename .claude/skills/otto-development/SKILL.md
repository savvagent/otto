...
> **Ported for Claude Code** from `.github/skills/otto-development/SKILL.md`, which is the canonical
...
# Otto-CLI Development — Autonomous End-to-End

Autonomous, plan-driven feature/fix workflow for the **otto** repository (a Rust-only,
...
This is the **otto-specific sibling** of `general-development`. Same spine; the
...
The spine mirrors what this repository builds: otto treats **everything as MCP-shaped** —
...
record; [`crates/otto-protocol/SPEC.md`](../../../crates/otto-protocol/SPEC.md) is the SPP
...
TUI actually spawns `otto-tool-fs` at runtime (a `cargo build` is a
prerequisite even for TUI-only work, since that binary is a `[[bin]]` target inside
`crates/otto` itself), that a plugin example (`examples/plugin-hello-*`) still loads,
...
6. **A public interface change is a deliberate, documented change.** The interfaces customers and
   agents bind to are the **SPP wire format** (`crates/otto-protocol/SPEC.md` — `CompleteRequest`,
   `CompleteResponse`, `StreamEvent`, content blocks), the **`ProviderHandler` / `ProviderClient`
   traits** (`otto-mcp`), the **tool MCP surfaces** (`tool-fs`, `tool-bash`, `tool-grep`,
   `tool-web`, `tool-lsp` — tool names and input schemas), the **plugin ABI**
   (`otto-plugin`/`otto-plugin-wit`/`otto-plugin-wasm`), the **slash-command surface**
   and **env vars** documented in `README.md`, and the **on-disk transcript/keyring formats**
   (`~/.otto/transcripts/*.json`, OS keyring entries under service `otto`). Additive
...
skipped. Add to the PR body: `Fast-path: no design spec per otto-development trivial-task
criteria — <reason>.`
...
| "I'll add a provider registry to `Host` for this one case"        | `Host` already owns a provider pool with proper APIs (`add_provider`/`remove_provider`/`set_active_provider`, `crates/otto-host/src/session.rs`) — route through those. Adding a *second*, ad hoc registry or hardcoding provider selection around the pool is the design defect the architect review will flag. |
...
| Convention            | Value                                                                                                                                                                                                                                                                                                                     |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Repo                   | `savvagent/otto` — pass `--repo savvagent/otto` on `gh` commands run from a worktree or outside the checkout.                                                                                                                                                                                          |
...
| Commit format          | `<scope>: <subject>` — scope is a crate directory or area, e.g. `otto-host:`, `otto-protocol:`, `otto-mcp:`, `provider-anthropic:`, `tool-fs:`, `tool-lsp:`, `otto-canvas:`, `docs:`, `ci:`, `release:` — for any crate not listed, use its own directory name under `crates/` as the scope (`provider-openai:`, `provider-local:`, `tool-bash:`, `tool-grep:`, `tool-web:`, `otto-fence:`, etc.). Squash-merge to `main` via PR.                                                                    |
...
| Conventions of record  | [`CLAUDE.md`](../../../CLAUDE.md) at the repo root is the primary conventions document — read it before any non-trivial change — but defer to the code where the two disagree (docs can lag the code; see the Load-Bearing Invariants note above). `PRD.md` is the vision/scope of record, `crates/otto-protocol/SPEC.md` is the wire-format spec of record, `README.md` documents developer-facing conventions. Read the code. |
| Build                  | `cargo build` (bare) only builds `crates/otto` (the workspace's `default-members`), but that crate's own `[[bin]]` targets include `otto-tool-fs`, so a bare `cargo build` is still required for TUI-only work (the TUI spawns that binary at runtime). To build the whole workspace, matching CI, use `cargo build --workspace --all-targets`. `cargo run -p otto` runs the TUI.                                                                                                                                                          |
| Test command           | `cargo test --workspace` (from the root). Per crate: `cargo test -p otto-host`, `cargo test -p otto-host -- name::of::test` for a single test. Headless smoke test: `cargo run -p otto-host --example headless -- "list my Cargo.toml"` (needs a provider — see README "Running providers as standalone MCP servers"). No database, no external services — tests are self-contained.                                                                                                             |
...
1. **Everything is MCP-shaped.** A provider is just a `ProviderHandler` (`otto-mcp`). Tools are
   ...
   standalone binary form (`otto-anthropic`, etc.) exists only for wire-protocol debugging, not
   as the primary path.
2. **The turn loop lives in `Host`.** `Host::run_turn_streaming` (in `otto-host`) loops
   ...
   (`OTTO.md`, if present) all live here — the TUI is a thin shell on top. A behavior change to
   the turn loop belongs in `otto-host`, not in `crates/otto`.
3. **The host-swap `RwLock` rule is never violated.** The TUI keeps the active host as
   ...
   `/connect` swaps the slot atomically. See `crates/otto/src/app.rs` and `crates/otto/src/tui.rs`.
...
4. **The provider transport split is a hard boundary per connection.** For any single provider
   ...
   Streamable HTTP transport) is opt-in, selected only when `OTTO_PROVIDER_URL` is set at
   connect time. `Host` holds a provider **pool** (`pool: RwLock<HashMap<ProviderId, PoolEntry>>`,
   `crates/otto-host/src/session.rs`) with an `active_provider` pointer that turns are routed to
...
5. **Tools are dispatched through `ToolRegistry`, which owns two paths: stdio child processes (the
   ...
   children map — see `crates/otto-host/src/tools.rs`). New *external* tools are wired via
   `HostConfig::with_tool` in `crates/otto/src/main.rs`, mirroring `crates/tool-fs`'s
...
7. **Secrets never touch disk in plaintext.** API keys live in the OS keyring under service
   `otto`, account `<provider id>`; `/connect` is the only writer. Never log a key, never echo
...
8. **Sandboxed execution is a real security boundary, not a courtesy.** `tool-bash` runs arbitrary
   ...
   (`otto-plugin-wasm`/`otto-plugin-wit`) runs inside the process. Any change that widens
...
otto uses **GitHub Issues when an issue exists, ticketless otherwise.** There is no JIRA.
...
> (`gh label create status:in-progress --repo savvagent/otto`, likewise `status:in-review`)
...
| ------------------- | ------------------------------------------------------------------------------------- | --------------------------------------------------------------------------- |
| Ref form            | `savvagent/otto#123` (`#123` short)                                          | the captured task brief                                                    |
| Intake / read AC    | `gh issue view <n> --repo savvagent/otto --json title,body,labels`           | the user's instruction, captured verbatim in working memory + the PR body  |
| → In Progress       | `gh issue edit <n> --repo savvagent/otto --add-label status:in-progress` (if the label exists; otherwise skip) | n/a — capture `T_impl_start` only                                          |
| → In Review         | `gh issue edit <n> --repo savvagent/otto --add-label status:in-review` (if the label exists; otherwise skip)   | n/a                                                                        |
...
Create a todo (via the orchestrating session's own `TodoWrite` tool — a capability of the CLI
running this skill, not anything documented in this repo) for each phase (1–6),
plus one per plan task once the plan exists, and update `status` as you go (`pending` →
`in_progress` → `completed`). `TodoWrite` has no dependency field, so encode ordering by keeping the
list in phase order and by never marking a task `in_progress` before its predecessor is `completed`
— Phase 2 after Phase 1, each implementation task after the plan is committed, etc. A blocked item
stays `in_progress` with the blocker named in its text. If the environment running this skill
...
This skill is written for the *orchestrating* Claude Code session's `Agent` tool — the CLI agent
running this skill — and **not** for the in-product `task` tool that otto itself (the software being
built) exposes to its own model via `.otto/agents/` markdown files (`README.md`'s "The `task` tool"
section, `{ description, prompt, subagent_type }`, depth-capped by `OTTO_AGENT_MAX_DEPTH`). Those
are two unrelated dispatch mechanisms that happen to share a parameter name — `Agent` belongs to the
orchestrating CLI (this skill's dispatch mechanism), while otto's `task` tool is a runtime feature of
the product under development and populates its own `subagent_type` enum from discovered agent files
...
| ----------------------------------------------------------------- | ---------------------- | --------------------------------------------------------------------------------------------------------------------- |
| Spec critique, plan critique (Phase 1 step 4, Phase 2 step 6)      | `subagent_type: "general-purpose"` | Claude Code has no built-in critique/rubber-duck agent, so the critique prompt itself carries the role: the templates open by casting the agent as a spec/plan reviewer that catches logic/design flaws and ignores style. Do not substitute a code reviewer. |
...
`architect-reviewer`, `code-reviewer`, `security-auditor`). Those are an **optional upgrade** for the
review trio, never a requirement: this skill must run unchanged in a fresh clone, so every mandatory
step above resolves to a built-in.

**One call, or several in one message.** Dispatch sequential, order-dependent steps (implementer
tasks, spec/plan critique loops, spec-compliance review) as a **single `Agent` call** — the call
returns the agent's report, and you need that result before deciding the next action. The mandatory
review trio (Phase 4 step 8) is three **independent** read-only passes: issue all three `Agent`
calls **in the same message** so they run concurrently rather than one after another. Concurrent
agents run in the background; each one's result arrives as a task-completion notification, so you
can keep working on something else (e.g. drafting the PR-comment aggregation skeleton) while they
run. Collect a result from its completion notification, or continue a still-running agent with
`SendMessage` addressed to it by name/ID. Never predict or invent a pending agent's findings — if a
notification has not arrived, the pass has not reported yet.

**Progress tracking** is `TodoWrite` (see Phase 0), not a per-dispatch field.

**Model selection.** Where the plan/spec calls for "mechanical, 1–2-file task" pass
`model: "haiku"`. For multi-file integration work or design-judgment tasks, omit `model` so the
dispatch inherits strong reasoning — but note that an omitted `model` resolves to the agent
definition's model first, then the configured default subagent model, and only then the parent
session's, so omission **alone does not guarantee** strong reasoning. Where the session's default
subagent model is known to be weaker than the orchestrator's, pass `model: "opus"` explicitly on
design-judgment dispatches. There is no `reasoning_effort` parameter — model choice is the only
effort dial.
...
Every `Agent` dispatch in this skill is issued by the top-level orchestrating session, never by
another dispatched agent trying to spawn its own subagents — so there is nothing to collapse or defer
here. If a dispatched `general-purpose` agent reports that a review or sub-task genuinely needs
further isolated investigation, the orchestrating session (you) issues that follow-up `Agent` call
directly.
...
- **GitHub:** `gh issue view <n> --repo savvagent/otto --json title,body,labels` — the body
  is the AC.
...
- **GitHub:** `gh issue edit <n> --repo savvagent/otto --add-label status:in-progress` (only
...
- **Approach** — the shape of the fix, referencing the actual crate layout (`crates/otto`,
  `otto-host`, `otto-protocol`, `otto-mcp`, `provider-*`, `tool-*`, `otto-plugin*`,
  `otto-canvas`) and citing `file:line` for existing code the design touches
...
`subagent_type: "general-purpose"` (the prompt carries the critique role). Fill `<PASTE FULL SPEC TEXT>` from the committed spec and cite the
...
- One `## Task N: <name>` per task, listing **Files:** (Create/Modify), then `- [ ]` steps in
  **failing-test-first order**: write failing test → run → implement → run → commit. Include exact
  file paths, exact commands (`cargo test -p otto-host`, `cargo test -p otto-host -- <test
...
- **A step verifying the host-swap `RwLock` rule** whenever the task touches `crates/otto/src/app.rs`
  or `crates/otto/src/tui.rs` — no `.await` may execute while the read guard is held
...
that file now and paste the template verbatim.** `subagent_type: "general-purpose"` (the prompt carries the critique role). Fill `<PASTE FULL
...
Create one todo (via `TodoWrite`) per task. **Do NOT make implementer subagents re-read the plan** —
...
Format` are load-bearing). `subagent_type: "general-purpose"`, a single `Agent` call. Fill the task text, the Context block
...
`subagent_type: "general-purpose"`, a single `Agent` call. Fill the task text and the implementer's report verbatim.
...
**read that file now and paste the template verbatim** — the template is what turns a
`general-purpose` agent into a read-only diff reviewer, so improvising the body loses the review.
`subagent_type: "general-purpose"`, a single `Agent` call; the built-in `/code-review` skill on the
same range is an acceptable substitute. Fill `<BASE_SHA>`/`<HEAD_SHA>` and the task text.
...
**read that file now and paste the template verbatim.** `subagent_type: "general-purpose"` with that
template (or the built-in `/code-review` skill on the same range). Fill the full
...
- [ ] cargo fmt --all --check
- [ ] cargo test -p otto-host --test cross_vendor_history --no-fail-fast (only if a provider/host-plumbing surface changed)
...
**Mark In Review** (`gh issue edit <n> --repo savvagent/otto --add-label status:in-review`,
...
PR gets ALL THREE dispatched via the `Agent` tool, issued in parallel (all three calls in the same
message, so they run concurrently and report back as task notifications), regardless of size:

| Reviewer                          | Claude Code dispatch        | Why                                                                                                                                                                                                                                                                                                             |
| ----------------------------------- | ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
rendering-focused pass for `otto-canvas`/Blitz changes). The independent security pass above
...
gh api repos/savvagent/otto/pulls/<PR>/comments
...
> (`gh run list --repo savvagent/otto --branch <branch> --limit 5`) and track THAT id —
...
```bash
gh run list --repo savvagent/otto --branch main --limit 5   # find the run for YOUR merge SHA
gh run watch <run-id> --repo savvagent/otto
```
...
- **TUI runtime wiring** — if `crates/otto/src/main.rs`, `crates/otto/src/app.rs`, or `crates/otto/src/tui.rs` changed:
  `cargo build --workspace` (the TUI spawns `otto-tool-fs` at runtime and needs the binary to
  exist), then `cargo run -p otto` and exercise the changed surface manually.
- **A new/changed provider** — if `crates/provider-*` changed: run the headless smoke test —
  `cargo run -p otto-host --example headless -- "list my Cargo.toml"` — with the appropriate
...
- **A new/changed tool** — if `crates/tool-*` changed: confirm `HostConfig::with_tool` wiring in
  `crates/otto/src/main.rs` still resolves the binary (via `$PATH` or its
  `OTTO_TOOL_*_BIN` override), and that the tool's MCP input schema still validates.
- **Plugin ABI** — if `crates/otto-plugin*` changed: build and load at least one example plugin
  (`examples/plugin-hello-interactive`, `plugin-hello-provider`, `plugin-hello-static`) and confirm
  it still loads and runs.
- **`otto-canvas`/Blitz** — if `crates/otto-canvas` changed: confirm it still builds on
...
- **GitHub:** `gh issue close <n> --repo savvagent/otto --comment "<summary>"` (the merged
  PR's `Closes #N` may have closed it already — verify with
  `gh issue view <n> --repo savvagent/otto --json state`).
...
| Implementer dispatch                                       | 1 `Agent` call (`general-purpose`) with full task text inline                                    | NEVER                                                  |
